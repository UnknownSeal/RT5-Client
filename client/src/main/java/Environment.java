	...
	public Environment(@OriginalArg(0) Buffer buffer) {
		...
		if (Static218.aClass177_Sub1_2.aBoolean364 && Static65.aClass19_3.method2886() > 0) {
			if ((flags & 0x1) == 0) {
				...
				this.anInt637 = buffer.mg4();
			}
			if ((flags & 0x2) == 0) {
				this.aFloat25 = 1.1523438F;
			} else {
				this.aFloat25 = (float) buffer.g2() / 256.0F;
			}
			if ((flags & 0x4) == 0) {
				this.aFloat23 = 0.69921875F;
			} else {
				this.aFloat23 = (float) buffer.g2() / 256.0F;
			}
			if ((flags & 0x8) == 0) {
				this.aFloat24 = 1.2F;
			} else {
				this.aFloat24 = (float) buffer.g2() / 256.0F;
			}
		} else {
			if ((flags & 0x1) != 0) {
				buffer.mg4();
		...
		if ((flags & 0x10) == 0) {
			...
			this.anInt632 = buffer.g2b_dup();
			this.anInt636 = buffer.g2b_dup();
			this.anInt640 = buffer.g2b_dup();
		}
		if ((flags & 0x20) == 0) {
			this.anInt641 = Static279.anInt5161;
		} else {
			this.anInt641 = buffer.mg4();
		}
		if ((flags & 0x40) == 0) {
			this.anInt634 = 0;
		} else {
			this.anInt634 = buffer.g2();
		}
		if ((flags & 0x80) == 0) {
			this.aClass42_1 = Static62.aClass42_5;
		} else {
			this.aClass42_1 = Static297.method5106(buffer.g2(), buffer.g2(), buffer.g2(), buffer.g2(), buffer.g2(), buffer.g2());
		}
	}
