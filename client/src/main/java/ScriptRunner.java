import java.util.Calendar;
import java.util.Date;
import org.openrs2.deob.annotation.OriginalArg;
import org.openrs2.deob.annotation.OriginalMember;
import org.openrs2.deob.annotation.Pc;

public final class ScriptRunner {

	@OriginalMember(owner = "client!lk", name = "h", descriptor = "[I")
	private static int[] intLocals;

	@OriginalMember(owner = "client!lk", name = "o", descriptor = "[Ljava/lang/String;")
	private static String[] stringLocals;

	@OriginalMember(owner = "client!lk", name = "r", descriptor = "Lclient!gr;")
	private static QuickChatPhrase activePhrase;

	@OriginalMember(owner = "client!lk", name = "v", descriptor = "Lclient!nk;")
	private static Component primaryActiveComponent;

	@OriginalMember(owner = "client!lk", name = "w", descriptor = "Lclient!nk;")
	private static Component secondaryActiveComponent;

	@OriginalMember(owner = "client!lk", name = "a", descriptor = "I")
	private static int isp = 0;

	@OriginalMember(owner = "client!lk", name = "b", descriptor = "[Ljava/lang/String;")
	private static final String[] stringStack = new String[1000];

	@OriginalMember(owner = "client!lk", name = "d", descriptor = "[Lclient!ol;")
	private static final Class172[] aClass172Array1 = new Class172[50];

	@OriginalMember(owner = "client!lk", name = "e", descriptor = "I")
	private static int anInt3652 = 0;

	@OriginalMember(owner = "client!lk", name = "k", descriptor = "[I")
	private static final int[] intStack = new int[1000];

	@OriginalMember(owner = "client!lk", name = "q", descriptor = "I")
	private static int anInt3661 = 0;

	@OriginalMember(owner = "client!lk", name = "s", descriptor = "Ljava/util/Calendar;")
	private static final Calendar calendar = Calendar.getInstance();

	@OriginalMember(owner = "client!lk", name = "t", descriptor = "[[I")
	private static final int[][] arrays = new int[5][5000];

	@OriginalMember(owner = "client!lk", name = "y", descriptor = "[I")
	private static final int[] anIntArray214 = new int[5];

	@OriginalMember(owner = "client!lk", name = "z", descriptor = "[I")
	private static final int[] anIntArray215 = new int[3];

	@OriginalMember(owner = "client!lk", name = "A", descriptor = "Lclient!hu;")
	public static final SoftLruHashTable A_SOFT_LRU_HASH_TABLE___30 = new SoftLruHashTable(4);

	@OriginalMember(owner = "client!lk", name = "B", descriptor = "[Ljava/lang/String;")
	private static final String[] aStringArray31 = new String[] { "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec" };

	@OriginalMember(owner = "client!lk", name = "a", descriptor = "(I)V")
	private static void method3589(@OriginalArg(0) int arg0) {
		@Pc(3) Component local3 = Static6.getComponent(arg0);
		if (local3 == null) {
			return;
		}
		@Pc(9) int local9 = arg0 >>> 16;
		@Pc(13) Component[] local13 = Static113.aComponentArrayArray2[local9];
		if (local13 == null) {
			@Pc(19) Component[] local19 = Static26.aComponentArrayArray1[local9];
			@Pc(22) int local22 = local19.length;
			local13 = Static113.aComponentArrayArray2[local9] = new Component[local22];
			Static403.method4608(local19, 0, local13, 0, local19.length);
		}
		@Pc(38) int local38;
		for (local38 = 0; local38 < local13.length && local13[local38] != local3; local38++) {
		}
		if (local38 >= local13.length) {
			return;
		}
		Static403.method4608(local13, local38 + 1, local13, local38, local13.length - local38 - 1);
		local13[local13.length - 1] = local3;
	}

	@OriginalMember(owner = "client!lk", name = "a", descriptor = "(Lclient!fn;)V")
	public static void method3590(@OriginalArg(0) HookRequest arg0) {
		method3592(arg0, 200000);
	}

	@OriginalMember(owner = "client!lk", name = "a", descriptor = "(Ljava/lang/String;I)V")
	private static void method3591(@OriginalArg(0) String arg0, @OriginalArg(1) int arg1) {
		if (Static4.staffModLevel == 0 && (Static56.playerUnderage && !Static101.aBoolean159 || Static109.enabled)) {
			return;
		}
		@Pc(11) String local11 = arg0.toLowerCase();
		@Pc(13) byte local13 = 0;
		if (local11.startsWith(LocalisedText.YELLOW.getLocalisedText(0))) {
			local13 = 0;
			arg0 = arg0.substring(LocalisedText.YELLOW.getLocalisedText(0).length());
		} else if (local11.startsWith(LocalisedText.RED.getLocalisedText(0))) {
			local13 = 1;
			arg0 = arg0.substring(LocalisedText.RED.getLocalisedText(0).length());
		} else if (local11.startsWith(LocalisedText.GREEN.getLocalisedText(0))) {
			local13 = 2;
			arg0 = arg0.substring(LocalisedText.GREEN.getLocalisedText(0).length());
		} else if (local11.startsWith(LocalisedText.CYAN.getLocalisedText(0))) {
			local13 = 3;
			arg0 = arg0.substring(LocalisedText.CYAN.getLocalisedText(0).length());
		} else if (local11.startsWith(LocalisedText.PURPLE.getLocalisedText(0))) {
			local13 = 4;
			arg0 = arg0.substring(LocalisedText.PURPLE.getLocalisedText(0).length());
		} else if (local11.startsWith(LocalisedText.WHITE.getLocalisedText(0))) {
			local13 = 5;
			arg0 = arg0.substring(LocalisedText.WHITE.getLocalisedText(0).length());
		} else if (local11.startsWith(LocalisedText.FLASH1.getLocalisedText(0))) {
			local13 = 6;
			arg0 = arg0.substring(LocalisedText.FLASH1.getLocalisedText(0).length());
		} else if (local11.startsWith(LocalisedText.FLASH2.getLocalisedText(0))) {
			local13 = 7;
			arg0 = arg0.substring(LocalisedText.FLASH2.getLocalisedText(0).length());
		} else if (local11.startsWith(LocalisedText.FLASH3.getLocalisedText(0))) {
			local13 = 8;
			arg0 = arg0.substring(LocalisedText.FLASH3.getLocalisedText(0).length());
		} else if (local11.startsWith(LocalisedText.GLOW1.getLocalisedText(0))) {
			local13 = 9;
			arg0 = arg0.substring(LocalisedText.GLOW1.getLocalisedText(0).length());
		} else if (local11.startsWith(LocalisedText.GLOW2.getLocalisedText(0))) {
			local13 = 10;
			arg0 = arg0.substring(LocalisedText.GLOW2.getLocalisedText(0).length());
		} else if (local11.startsWith(LocalisedText.GLOW3.getLocalisedText(0))) {
			local13 = 11;
			arg0 = arg0.substring(LocalisedText.GLOW3.getLocalisedText(0).length());
		} else if (client.language != 0) {
			if (local11.startsWith(LocalisedText.YELLOW.getLocalisedText(client.language))) {
				local13 = 0;
				arg0 = arg0.substring(LocalisedText.YELLOW.getLocalisedText(client.language).length());
			} else if (local11.startsWith(LocalisedText.RED.getLocalisedText(client.language))) {
				local13 = 1;
				arg0 = arg0.substring(LocalisedText.RED.getLocalisedText(client.language).length());
			} else if (local11.startsWith(LocalisedText.GREEN.getLocalisedText(client.language))) {
				local13 = 2;
				arg0 = arg0.substring(LocalisedText.GREEN.getLocalisedText(client.language).length());
			} else if (local11.startsWith(LocalisedText.CYAN.getLocalisedText(client.language))) {
				local13 = 3;
				arg0 = arg0.substring(LocalisedText.CYAN.getLocalisedText(client.language).length());
			} else if (local11.startsWith(LocalisedText.PURPLE.getLocalisedText(client.language))) {
				local13 = 4;
				arg0 = arg0.substring(LocalisedText.PURPLE.getLocalisedText(client.language).length());
			} else if (local11.startsWith(LocalisedText.WHITE.getLocalisedText(client.language))) {
				local13 = 5;
				arg0 = arg0.substring(LocalisedText.WHITE.getLocalisedText(client.language).length());
			} else if (local11.startsWith(LocalisedText.FLASH1.getLocalisedText(client.language))) {
				local13 = 6;
				arg0 = arg0.substring(LocalisedText.FLASH1.getLocalisedText(client.language).length());
			} else if (local11.startsWith(LocalisedText.FLASH2.getLocalisedText(client.language))) {
				local13 = 7;
				arg0 = arg0.substring(LocalisedText.FLASH2.getLocalisedText(client.language).length());
			} else if (local11.startsWith(LocalisedText.FLASH3.getLocalisedText(client.language))) {
				local13 = 8;
				arg0 = arg0.substring(LocalisedText.FLASH3.getLocalisedText(client.language).length());
			} else if (local11.startsWith(LocalisedText.GLOW1.getLocalisedText(client.language))) {
				local13 = 9;
				arg0 = arg0.substring(LocalisedText.GLOW1.getLocalisedText(client.language).length());
			} else if (local11.startsWith(LocalisedText.GLOW2.getLocalisedText(client.language))) {
				local13 = 10;
				arg0 = arg0.substring(LocalisedText.GLOW2.getLocalisedText(client.language).length());
			} else if (local11.startsWith(LocalisedText.GLOW3.getLocalisedText(client.language))) {
				local13 = 11;
				arg0 = arg0.substring(LocalisedText.GLOW3.getLocalisedText(client.language).length());
			}
		}
		local11 = arg0.toLowerCase();
		@Pc(451) byte local451 = 0;
		if (local11.startsWith(LocalisedText.WAVE.getLocalisedText(0))) {
			local451 = 1;
			arg0 = arg0.substring(LocalisedText.WAVE.getLocalisedText(0).length());
		} else if (local11.startsWith(LocalisedText.WAVE2.getLocalisedText(0))) {
			local451 = 2;
			arg0 = arg0.substring(LocalisedText.WAVE2.getLocalisedText(0).length());
		} else if (local11.startsWith(LocalisedText.SHAKE.getLocalisedText(0))) {
			local451 = 3;
			arg0 = arg0.substring(LocalisedText.SHAKE.getLocalisedText(0).length());
		} else if (local11.startsWith(LocalisedText.SCROLL.getLocalisedText(0))) {
			local451 = 4;
			arg0 = arg0.substring(LocalisedText.SCROLL.getLocalisedText(0).length());
		} else if (local11.startsWith(LocalisedText.SLIDE.getLocalisedText(0))) {
			local451 = 5;
			arg0 = arg0.substring(LocalisedText.SLIDE.getLocalisedText(0).length());
		} else if (client.language != 0) {
			if (local11.startsWith(LocalisedText.WAVE.getLocalisedText(client.language))) {
				local451 = 1;
				arg0 = arg0.substring(LocalisedText.WAVE.getLocalisedText(client.language).length());
			} else if (local11.startsWith(LocalisedText.WAVE2.getLocalisedText(client.language))) {
				local451 = 2;
				arg0 = arg0.substring(LocalisedText.WAVE2.getLocalisedText(client.language).length());
			} else if (local11.startsWith(LocalisedText.SHAKE.getLocalisedText(client.language))) {
				local451 = 3;
				arg0 = arg0.substring(LocalisedText.SHAKE.getLocalisedText(client.language).length());
			} else if (local11.startsWith(LocalisedText.SCROLL.getLocalisedText(client.language))) {
				local451 = 4;
				arg0 = arg0.substring(LocalisedText.SCROLL.getLocalisedText(client.language).length());
			} else if (local11.startsWith(LocalisedText.SLIDE.getLocalisedText(client.language))) {
				local451 = 5;
				arg0 = arg0.substring(LocalisedText.SLIDE.getLocalisedText(client.language).length());
			}
		}
		Static91.method1960(Static102.aClass145_141);
		Static257.outboundBuffer.p1(0);
		@Pc(646) int local646 = Static257.outboundBuffer.offset;
		if (arg1 == 5021) {
			Static257.outboundBuffer.p1(1);
		} else {
			Static257.outboundBuffer.p1(0);
		}
		Static257.outboundBuffer.p1(local13);
		Static257.outboundBuffer.p1(local451);
		Static120.method2379(arg0, Static257.outboundBuffer);
		Static257.outboundBuffer.psize1(Static257.outboundBuffer.offset - local646);
	}

	@OriginalMember(owner = "client!lk", name = "a", descriptor = "(Lclient!fn;I)V")
	private static void method3592(@OriginalArg(0) HookRequest request, @OriginalArg(1) int maxCycles) {
		@Pc(2) Object[] arguments = request.arguments;
		@Pc(8) int scriptID = (Integer) arguments[0];
		@Pc(12) ClientScript script = Static295.get(scriptID);
		if (script == null) {
			return;
		}
		intLocals = new int[script.intLocals];
		@Pc(21) int intLocalIndex = 0;
		stringLocals = new String[script.stringLocals];
		@Pc(27) int stringLocalIndex = 0;
		for (@Pc(29) int i = 1; i < arguments.length; i++) {
			if (arguments[i] instanceof Integer) {
				@Pc(41) int value = (Integer) arguments[i];
				if (value == 0x80000001) {
					value = request.mouseX;
				}
				if (value == 0x80000002) {
					value = request.mouseY;
				}
				if (value == 0x80000003) {
					value = request.source == null ? -1 : request.source.id;
				}
				if (value == 0x80000004) {
					value = request.op;
				}
				if (value == 0x80000005) {
					value = request.source == null ? -1 : request.source.createdComponentID;
				}
				if (value == 0x80000006) {
					value = request.target == null ? -1 : request.target.id;
				}
				if (value == 0x80000007) {
					value = request.target == null ? -1 : request.target.createdComponentID;
				}
				if (value == 0x80000008) {
					value = request.keyCode;
				}
				if (value == 0x80000009) {
					value = request.keyChar;
				}
				intLocals[intLocalIndex++] = value;
			} else if (arguments[i] instanceof String) {
				@Pc(135) String value = (String) arguments[i];
				if (value.equals("event_opbase")) {
					value = request.opBase;
				}
				stringLocals[stringLocalIndex++] = value;
			}
		}
		run(script, maxCycles);
	}

	@OriginalMember(owner = "client!lk", name = "b", descriptor = "(I)Ljava/lang/String;")
	private static String method3593(@OriginalArg(0) int arg0) {
		@Pc(6) long local6 = ((long) arg0 + 11745L) * 86400000L;
		calendar.setTime(new Date(local6));
		@Pc(16) int local16 = calendar.get(5);
		@Pc(20) int local20 = calendar.get(2);
		@Pc(24) int local24 = calendar.get(1);
		return local16 + "-" + aStringArray31[local20] + "-" + local24;
	}

	@OriginalMember(owner = "client!lk", name = "a", descriptor = "(IZ)V")
	private static void run(@OriginalArg(0) int opcode, @OriginalArg(1) boolean secondary) {
		@Pc(137) Component component;
		@Pc(25) int local25 ;
		@Pc(35) Component local35;
		@Pc(56) int local56;
		@Pc(19) int int3;
		@Pc(210) Component local210;
		@Pc(13) int int1;
		if (opcode < 300) {
			if (opcode == 100) {
				isp -= 3;
				int1 = intStack[isp];
				int3 = intStack[isp + 1];
				local25  = intStack[isp + 2];
				if (int3 == 0) {
					throw new RuntimeException();
				}
				local35 = Static6.getComponent(int1);
				if (local35.createdComponent == null) {
					local35.createdComponent = new Component[local25  + 1];
				}
				if (local35.createdComponent.length <= local25 ) {
					@Pc(54) Component[] local54 = new Component[local25  + 1];
					for (local56 = 0; local56 < local35.createdComponent.length; local56++) {
						local54[local56] = local35.createdComponent[local56];
					}
					local35.createdComponent = local54;
				}
				if (local25  > 0 && local35.createdComponent[local25  - 1] == null) {
					throw new RuntimeException("Gap at:" + (local25  - 1));
				}
				@Pc(99) Component local99 = new Component();
				local99.type = int3;
				local99.layer = local99.id = local35.id;
				local99.createdComponentID = local25 ;
				local35.createdComponent[local25 ] = local99;
				if (secondary) {
					secondaryActiveComponent = local99;
				} else {
					primaryActiveComponent = local99;
				}
				Static200.redraw(local35);
				return;
			}
			@Pc(158) Component local158;
			if (opcode == 101) {
				component = secondary ? secondaryActiveComponent : primaryActiveComponent;
				if (component.createdComponentID == -1) {
					if (secondary) {
						throw new RuntimeException("Tried to .cc_delete static .active-component!");
					}
					throw new RuntimeException("Tried to cc_delete static active-component!");
				}
				local158 = Static6.getComponent(component.id);
				local158.createdComponent[component.createdComponentID] = null;
				Static200.redraw(local158);
				return;
			}
			if (opcode == 102) {
				component = Static6.getComponent(intStack[--isp]);
				component.createdComponent = null;
				Static200.redraw(component);
				return;
			}
			if (opcode == 200) {
				isp -= 2;
				int1 = intStack[isp];
				int3 = intStack[isp + 1];
				local210 = Static207.getCreatedComponent(int1, int3);
				if (local210 != null && int3 != -1) {
					intStack[isp++] = 1;
					if (secondary) {
						secondaryActiveComponent = local210;
						return;
					}
					primaryActiveComponent = local210;
					return;
				}
				intStack[isp++] = 0;
				return;
			}
			if (opcode == 201) {
				int1 = intStack[--isp];
				local158 = Static6.getComponent(int1);
				if (local158 != null) {
					intStack[isp++] = 1;
					if (secondary) {
						secondaryActiveComponent = local158;
						return;
					}
					primaryActiveComponent = local158;
					return;
				}
				intStack[isp++] = 0;
				return;
			}
			if (opcode == 202) {
				int1 = intStack[--isp];
				method3589(int1);
				return;
			}
			if (opcode == 203) {
				int1 = intStack[--isp];
				method3596(int1);
				return;
			}
		} else {
			@Pc(353) int local353;
			@Pc(412) boolean local412;
			if (opcode < 500) {
				if (opcode == 403) {
					isp -= 2;
					int1 = intStack[isp];
					int3 = intStack[isp + 1];
					for (local25  = 0; local25  < PlayerAppearance.MALE_FEATURES.length; local25 ++) {
						if (PlayerAppearance.MALE_FEATURES[local25 ] == int1) {
							Static17.self.appearance.method3180(IdentityKitTypeList.identityKitTypeList, local25 , int3);
							return;
						}
					}
					for (local353 = 0; local353 < PlayerAppearance.FEMALE_FEATURES.length; local353++) {
						if (PlayerAppearance.FEMALE_FEATURES[local25 ] == int1) {
							Static17.self.appearance.method3180(IdentityKitTypeList.identityKitTypeList, local353, int3);
							return;
						}
					}
					return;
				}
				if (opcode == 404) {
					isp -= 2;
					int1 = intStack[isp];
					int3 = intStack[isp + 1];
					Static17.self.appearance.setColor(int1, int3);
					return;
				}
				if (opcode == 410) {
					local412 = intStack[--isp] != 0;
					Static17.self.appearance.setGender(local412);
					return;
				}
			} else if ((opcode < 1000 || opcode >= 1100) && (opcode < 2000 || opcode >= 2100)) {
				@Pc(1101) String local1101;
				if (opcode >= 1100 && opcode < 1200 || !(opcode < 2100 || opcode >= 2200)) {
					if (opcode >= 2000) {
						opcode -= 1000;
						component = Static6.getComponent(intStack[--isp]);
					} else {
						component = secondary ? secondaryActiveComponent : primaryActiveComponent;
					}
					if (opcode == 1100) {
						isp -= 2;
						component.scrollX = intStack[isp];
						if (component.scrollWidth - component.width < component.scrollX) {
							component.scrollX = component.scrollWidth - component.width;
						}
						if (component.scrollX < 0) {
							component.scrollX = 0;
						}
						component.scrollY = intStack[isp + 1];
						if (component.scrollY > component.scrollHeight - component.height) {
							component.scrollY = component.scrollHeight - component.height;
						}
						if (component.scrollY < 0) {
							component.scrollY = 0;
						}
						Static200.redraw(component);
						if (component.createdComponentID == -1) {
							Static369.setComponentScrollPositionClient(component.id);
						}
						return;
					}
					if (opcode == 1101) {
						component.color = intStack[--isp];
						Static200.redraw(component);
						if (component.createdComponentID == -1) {
							Static278.method4722(component.id);
						}
						return;
					}
					if (opcode == 1102) {
						component.fill = intStack[--isp] == 1;
						Static200.redraw(component);
						return;
					}
					if (opcode == 1103) {
						component.transparency = intStack[--isp];
						Static200.redraw(component);
						return;
					}
					if (opcode == 1104) {
						component.lineWidth = intStack[--isp];
						Static200.redraw(component);
						return;
					}
					if (opcode == 1105) {
						int3 = intStack[--isp];
						if (component.graphic != int3) {
							component.graphic = int3;
							Static200.redraw(component);
						}
						if (component.createdComponentID == -1) {
							Static137.method2552(component.id);
						}
						return;
					}
					if (opcode == 1106) {
						component.angle = intStack[--isp];
						Static200.redraw(component);
						return;
					}
					if (opcode == 1107) {
						component.tiled = intStack[--isp] == 1;
						Static200.redraw(component);
						return;
					}
					if (opcode == 1108) {
						component.modelType = 1;
						component.modelID = intStack[--isp];
						Static200.redraw(component);
						if (component.createdComponentID == -1) {
							DelayedStateChange.setComponentModelClient(component.id);
						}
						return;
					}
					if (opcode == 1109) {
						isp -= 6;
						component.modelXOffset = intStack[isp];
						component.modelZOffset = intStack[isp + 1];
						component.modelXAngle = intStack[isp + 2];
						component.modelYAngle = intStack[isp + 3];
						component.modelYOffset = intStack[isp + 4];
						component.modelZoom = intStack[isp + 5];
						Static200.redraw(component);
						if (component.createdComponentID == -1) {
							DelayedStateChange.setComponentModelAngleClient(component.id);
							DelayedStateChange.setComponentModelOffsetClient(component.id);
						}
						return;
					}
					if (opcode == 1110) {
						int3 = intStack[--isp];
						if (component.modelSeqID != int3) {
							component.modelSeqID = int3;
							component.anInt4247 = 0;
							component.anInt4316 = 1;
							component.anInt4283 = 0;
							Static200.redraw(component);
						}
						if (component.createdComponentID == -1) {
							DelayedStateChange.setComponentModelAnimClient(component.id);
						}
						return;
					}
					if (opcode == 1111) {
						component.modelOrthographic = intStack[--isp] == 1;
						Static200.redraw(component);
						return;
					}
					if (opcode == 1112) {
						local1101 = stringStack[--anInt3652];
						if (!local1101.equals(component.text)) {
							component.text = local1101;
							Static200.redraw(component);
						}
						if (component.createdComponentID == -1) {
							DelayedStateChange.setComponentTextClient(component.id);
						}
						return;
					}
					if (opcode == 1113) {
						component.textFont = intStack[--isp];
						Static200.redraw(component);
						if (component.createdComponentID == -1) {
							DelayedStateChange.method2439(component.id);
						}
						return;
					}
					if (opcode == 1114) {
						isp -= 3;
						component.halign = intStack[isp];
						component.valign = intStack[isp + 1];
						component.textLineHeight = intStack[isp + 2];
						Static200.redraw(component);
						return;
					}
					if (opcode == 1115) {
						component.textShadow = intStack[--isp] == 1;
						Static200.redraw(component);
						return;
					}
					if (opcode == 1116) {
						component.outline = intStack[--isp];
						Static200.redraw(component);
						return;
					}
					if (opcode == 1117) {
						component.graphicShadow = intStack[--isp];
						Static200.redraw(component);
						return;
					}
					if (opcode == 1118) {
						component.horizontalFlip = intStack[--isp] == 1;
						Static200.redraw(component);
						return;
					}
					if (opcode == 1119) {
						component.verticalFlip = intStack[--isp] == 1;
						Static200.redraw(component);
						return;
					}
					if (opcode == 1120) {
						isp -= 2;
						component.scrollWidth = intStack[isp];
						component.scrollHeight = intStack[isp + 1];
						Static200.redraw(component);
						if (component.type == 0) {
							Static85.method1911(false, component);
						}
						return;
					}
					if (opcode == 1121) {
						isp -= 2;
						component.aShort62 = (short) intStack[isp];
						component.aShort61 = (short) intStack[isp + 1];
						Static200.redraw(component);
						return;
					}
					if (opcode == 1122) {
						component.alpha = intStack[--isp] == 1;
						Static200.redraw(component);
						return;
					}
					if (opcode == 1123) {
						component.modelZoom = intStack[--isp];
						Static200.redraw(component);
						if (component.createdComponentID == -1) {
							DelayedStateChange.setComponentModelAngleClient(component.id);
						}
						return;
					}
					if (opcode == 1124) {
						int3 = intStack[--isp];
						component.lineDirection = int3 == 1;
						Static200.redraw(component);
						return;
					}
					if (opcode == 1125) {
						isp -= 2;
						component.anInt4307 = intStack[isp];
						component.anInt4253 = intStack[isp + 1];
						Static200.redraw(component);
						return;
					}
				} else if (opcode >= 1200 && opcode < 1300 || opcode >= 2200 && opcode < 2300) {
					if (opcode >= 2000) {
						opcode -= 1000;
						component = Static6.getComponent(intStack[--isp]);
					} else {
						component = secondary ? secondaryActiveComponent : primaryActiveComponent;
					}
					Static200.redraw(component);
					if (opcode == 1200 || opcode == 1205 || opcode == 1208 || opcode == 1209 || opcode == 1212 || opcode == 1213) {
						isp -= 2;
						int3 = intStack[isp];
						local25  = intStack[isp + 1];
						if (component.createdComponentID == -1) {
							DelayedStateChange.setComponentObjClient(component.id);
							DelayedStateChange.setComponentModelAngleClient(component.id);
							DelayedStateChange.setComponentModelOffsetClient(component.id);
						}
						if (int3 == -1) {
							component.modelType = 1;
							component.modelID = -1;
							component.objId = -1;
							return;
						}
						component.objId = int3;
						component.objCount = local25 ;
						if (opcode == 1208 || opcode == 1209) {
							component.objWearColor = true;
						} else {
							component.objWearColor = false;
						}
						@Pc(1537) ObjType objType = ObjTypeList.objTypeList.get(int3);
						component.modelXAngle = objType.xAngle2D;
						component.modelYAngle = objType.yAngle2D;
						component.modelYOffset = objType.zAngle2D;
						component.modelXOffset = objType.xOffset2D;
						component.modelZOffset = objType.yOffset2D;
						component.modelZoom = objType.zoom2d;
						if (opcode == 1205 || opcode == 1209) {
							component.anInt4306 = 0;
						} else if (opcode == 1212 || opcode == 1213) {
							component.anInt4306 = 1;
						} else {
							component.anInt4306 = 2;
						}
						if (component.anInt4251 > 0) {
							component.modelZoom = component.modelZoom * 32 / component.anInt4251;
							return;
						}
						if (component.anInt4294 > 0) {
							component.modelZoom = component.modelZoom * 32 / component.anInt4294;
						}
						return;
					}
					if (opcode == 1201) {
						component.modelType = 2;
						component.modelID = intStack[--isp];
						if (component.createdComponentID == -1) {
							DelayedStateChange.setComponentModelClient(component.id);
						}
						return;
					}
					if (opcode == 1202) {
						component.modelType = 3;
						component.modelID = -1;
						if (component.createdComponentID == -1) {
							DelayedStateChange.setComponentModelClient(component.id);
						}
						return;
					}
					if (opcode == 1203) {
						component.modelType = 6;
						component.modelID = intStack[--isp];
						if (component.createdComponentID == -1) {
							DelayedStateChange.setComponentModelClient(component.id);
						}
						return;
					}
					if (opcode == 1204) {
						component.modelType = 5;
						component.modelID = intStack[--isp];
						if (component.createdComponentID == -1) {
							DelayedStateChange.setComponentModelClient(component.id);
						}
						return;
					}
					if (opcode == 1206) {
						isp -= 4;
						component.anInt4280 = intStack[isp];
						component.anInt4267 = intStack[isp + 1];
						component.anInt4235 = intStack[isp + 2];
						component.anInt4242 = intStack[isp + 3];
						Static200.redraw(component);
						return;
					}
					if (opcode == 1207) {
						isp -= 2;
						component.anInt4270 = intStack[isp];
						component.anInt4302 = intStack[isp + 1];
						Static200.redraw(component);
						return;
					}
					if (opcode == 1210) {
						isp -= 4;
						component.modelID = intStack[isp];
						component.anInt4236 = intStack[isp + 1];
						if (intStack[isp + 2] == 1) {
							component.modelType = 9;
						} else {
							component.modelType = 8;
						}
						if (intStack[isp + 3] == 1) {
							component.objWearColor = true;
						} else {
							component.objWearColor = false;
						}
						if (component.createdComponentID == -1) {
							DelayedStateChange.setComponentModelClient(component.id);
						}
						return;
					}
					if (opcode == 1211) {
						component.modelType = 5;
						component.modelID = Static234.selfID;
						component.anInt4236 = 0;
						if (component.createdComponentID == -1) {
							DelayedStateChange.setComponentModelClient(component.id);
						}
						return;
					}
				} else if (opcode >= 1300 && opcode < 1400 || opcode >= 2300 && opcode < 2400) {
					if (opcode >= 2000) {
						opcode -= 1000;
						component = Static6.getComponent(intStack[--isp]);
					} else {
						component = secondary ? secondaryActiveComponent : primaryActiveComponent;
					}
					if (opcode == 1300) {
						int3 = intStack[--isp] - 1;
						if (int3 >= 0 && int3 <= 9) {
							component.setOp(int3, stringStack[--anInt3652]);
							return;
						}
						anInt3652--;
						return;
					}
					if (opcode == 1301) {
						isp -= 2;
						int3 = intStack[isp];
						local25  = intStack[isp + 1];
						component.dragComponent = Static207.getCreatedComponent(int3, local25 );
						return;
					}
					if (opcode == 1302) {
						int3 = intStack[--isp];
						if (int3 != Static164.anInt3138 && int3 != Static158.anInt2895 && int3 != Static83.anInt1838) {
							return;
						}
						component.anInt4285 = int3;
						return;
					}
					if (opcode == 1303) {
						component.dragDeadzone = intStack[--isp];
						return;
					}
					if (opcode == 1304) {
						component.dragDeadtime = intStack[--isp];
						return;
					}
					if (opcode == 1305) {
						component.opBase = stringStack[--anInt3652];
						return;
					}
					if (opcode == 1306) {
						component.opCircumfix = stringStack[--anInt3652];
						return;
					}
					if (opcode == 1307) {
						component.ops = null;
						return;
					}
					if (opcode == 1308) {
						component.anInt4286 = intStack[--isp];
						component.anInt4238 = intStack[--isp];
						return;
					}
					if (opcode == 1309) {
						int3 = intStack[--isp];
						local25  = intStack[--isp];
						if (local25  >= 1 && local25  <= 10) {
							component.method4109(int3, local25  - 1);
						}
						return;
					}
					if (opcode == 1310) {
						component.pausText = stringStack[--anInt3652];
						return;
					}
					if (opcode == 1311) {
						component.anInt4260 = intStack[--isp];
						return;
					}
				} else {
					@Pc(2194) int local2194;
					if (opcode >= 1400 && opcode < 1500 || opcode >= 2400 && opcode < 2500) {
						if (opcode >= 2000) {
							opcode -= 1000;
							component = Static6.getComponent(intStack[--isp]);
						} else {
							component = secondary ? secondaryActiveComponent : primaryActiveComponent;
						}
						local1101 = stringStack[--anInt3652];
						@Pc(2137) int[] local2137 = null;
						if (local1101.length() > 0 && local1101.charAt(local1101.length() - 1) == 'Y') {
							local353 = intStack[--isp];
							if (local353  > 0) {
								local2137 = new int[local353 ];
								while (local353 -- > 0) {
									local2137[local353 ] = intStack[--isp];
								}
							}
							local1101 = local1101.substring(0, local1101.length() - 1);
						}
						@Pc(2189) Object[] local2189 = new Object[local1101.length() + 1];
						for (local2194 = local2189.length - 1; local2194 >= 1; local2194--) {
							if (local1101.charAt(local2194 - 1) == 's') {
								local2189[local2194] = stringStack[--anInt3652];
							} else {
								local2189[local2194] = Integer.valueOf(intStack[--isp]);
							}
						}
						local56 = intStack[--isp];
						if (local56 == -1) {
							local2189 = null;
						} else {
							local2189[0] = Integer.valueOf(local56);
						}
						if (opcode == 1400) {
							component.onClickRepeat = local2189;
						} else if (opcode == 1401) {
							component.onHold = local2189;
						} else if (opcode == 1402) {
							component.onRelease = local2189;
						} else if (opcode == 1403) {
							component.onMouseOver = local2189;
						} else if (opcode == 1404) {
							component.onMouseLeave = local2189;
						} else if (opcode == 1405) {
							component.onDragStart = local2189;
						} else if (opcode == 1406) {
							component.onUseWith = local2189;
						} else if (opcode == 1407) {
							component.onVarpTransmit = local2189;
							component.varpTriggers = local2137;
						} else if (opcode == 1408) {
							component.onTimer = local2189;
						} else if (opcode == 1409) {
							component.onOptionClick = local2189;
						} else if (opcode == 1410) {
							component.onDragRelease = local2189;
						} else if (opcode == 1411) {
							component.onDrag = local2189;
						} else if (opcode == 1412) {
							component.onMouseRepeat = local2189;
						} else if (opcode == 1414) {
							component.onInvTransmit = local2189;
							component.inventoryTriggers = local2137;
						} else if (opcode == 1415) {
							component.onStatTransmit = local2189;
							component.statTriggers = local2137;
						} else if (opcode == 1416) {
							component.onUse = local2189;
						} else if (opcode == 1417) {
							component.onScroll = local2189;
						} else if (opcode == 1418) {
							component.onMsg = local2189;
						} else if (opcode == 1419) {
							component.onKey = local2189;
						} else if (opcode == 1420) {
							component.onFriendTransmit = local2189;
						} else if (opcode == 1421) {
							component.onClanTransmit = local2189;
						} else if (opcode == 1422) {
							component.onMiscTransmit = local2189;
						} else if (opcode == 1423) {
							component.onDialogAbort = local2189;
						} else if (opcode == 1424) {
							component.onWidgetsOpenClose = local2189;
						} else if (opcode == 1425) {
							component.onStockTransmit = local2189;
						} else if (opcode == 1426) {
							component.onMinimapUnlock = local2189;
						} else if (opcode == 1427) {
							component.onResize = local2189;
						} else if (opcode == 1428) {
							component.onVarcTransmit = local2189;
							component.varcTriggers = local2137;
						} else if (opcode == 1429) {
							component.onVarcstrTransmit = local2189;
							component.varcstrTriggers = local2137;
						}
						component.aBoolean296 = true;
						return;
					}
					if (opcode < 1600) {
						component = secondary ? secondaryActiveComponent : primaryActiveComponent;
						if (opcode == 1500) {
							intStack[isp++] = component.x;
							return;
						}
						if (opcode == 1501) {
							intStack[isp++] = component.y;
							return;
						}
						if (opcode == 1502) {
							intStack[isp++] = component.width;
							return;
						}
						if (opcode == 1503) {
							intStack[isp++] = component.height;
							return;
						}
						if (opcode == 1504) {
							intStack[isp++] = component.hidden ? 1 : 0;
							return;
						}
						if (opcode == 1505) {
							intStack[isp++] = component.layer;
							return;
						}
					} else if (opcode < 1700) {
						component = secondary ? secondaryActiveComponent : primaryActiveComponent;
						if (opcode == 1600) {
							intStack[isp++] = component.scrollX;
							return;
						}
						if (opcode == 1601) {
							intStack[isp++] = component.scrollY;
							return;
						}
						if (opcode == 1602) {
							stringStack[anInt3652++] = component.text;
							return;
						}
						if (opcode == 1603) {
							intStack[isp++] = component.scrollWidth;
							return;
						}
						if (opcode == 1604) {
							intStack[isp++] = component.scrollHeight;
							return;
						}
						if (opcode == 1605) {
							intStack[isp++] = component.modelZoom;
							return;
						}
						if (opcode == 1606) {
							intStack[isp++] = component.modelXAngle;
							return;
						}
						if (opcode == 1607) {
							intStack[isp++] = component.modelYOffset;
							return;
						}
						if (opcode == 1608) {
							intStack[isp++] = component.modelYAngle;
							return;
						}
						if (opcode == 1609) {
							intStack[isp++] = component.transparency;
							return;
						}
						if (opcode == 1610) {
							intStack[isp++] = component.modelXOffset;
							return;
						}
						if (opcode == 1611) {
							intStack[isp++] = component.modelZOffset;
							return;
						}
						if (opcode == 1612) {
							intStack[isp++] = component.graphic;
							return;
						}
					} else if (opcode < 1800) {
						component = secondary ? secondaryActiveComponent : primaryActiveComponent;
						if (opcode == 1700) {
							intStack[isp++] = component.objId;
							return;
						}
						if (opcode == 1701) {
							if (component.objId != -1) {
								intStack[isp++] = component.objCount;
								return;
							}
							intStack[isp++] = 0;
							return;
						}
						if (opcode == 1702) {
							intStack[isp++] = component.createdComponentID;
							return;
						}
					} else if (opcode < 1900) {
						component = secondary ? secondaryActiveComponent : primaryActiveComponent;
						if (opcode == 1800) {
							intStack[isp++] = Static45.getServerActiveProperties(component).getTargetMask();
							return;
						}
						if (opcode == 1801) {
							int3 = intStack[--isp];
							int3--;
							if (component.ops != null && int3 < component.ops.length && component.ops[int3] != null) {
								stringStack[anInt3652++] = component.ops[int3];
								return;
							}
							stringStack[anInt3652++] = "";
							return;
						}
						if (opcode == 1802) {
							if (component.opBase == null) {
								stringStack[anInt3652++] = "";
								return;
							}
							stringStack[anInt3652++] = component.opBase;
							return;
						}
					} else if (opcode < 2600) {
						component = Static6.getComponent(intStack[--isp]);
						if (opcode == 2500) {
							intStack[isp++] = component.x;
							return;
						}
						if (opcode == 2501) {
							intStack[isp++] = component.y;
							return;
						}
						if (opcode == 2502) {
							intStack[isp++] = component.width;
							return;
						}
						if (opcode == 2503) {
							intStack[isp++] = component.height;
							return;
						}
						if (opcode == 2504) {
							intStack[isp++] = component.hidden ? 1 : 0;
							return;
						}
						if (opcode == 2505) {
							intStack[isp++] = component.layer;
							return;
						}
					} else if (opcode < 2700) {
						component = Static6.getComponent(intStack[--isp]);
						if (opcode == 2600) {
							intStack[isp++] = component.scrollX;
							return;
						}
						if (opcode == 2601) {
							intStack[isp++] = component.scrollY;
							return;
						}
						if (opcode == 2602) {
							stringStack[anInt3652++] = component.text;
							return;
						}
						if (opcode == 2603) {
							intStack[isp++] = component.scrollWidth;
							return;
						}
						if (opcode == 2604) {
							intStack[isp++] = component.scrollHeight;
							return;
						}
						if (opcode == 2605) {
							intStack[isp++] = component.modelZoom;
							return;
						}
						if (opcode == 2606) {
							intStack[isp++] = component.modelXAngle;
							return;
						}
						if (opcode == 2607) {
							intStack[isp++] = component.modelYOffset;
							return;
						}
						if (opcode == 2608) {
							intStack[isp++] = component.modelYAngle;
							return;
						}
						if (opcode == 2609) {
							intStack[isp++] = component.transparency;
							return;
						}
						if (opcode == 2610) {
							intStack[isp++] = component.modelXOffset;
							return;
						}
						if (opcode == 2611) {
							intStack[isp++] = component.modelZOffset;
							return;
						}
						if (opcode == 2612) {
							intStack[isp++] = component.graphic;
							return;
						}
					} else {
						@Pc(3356) SubInterface local3356;
						@Pc(3254) SubInterface local3254;
						if (opcode < 2800) {
							if (opcode == 2700) {
								component = Static6.getComponent(intStack[--isp]);
								intStack[isp++] = component.objId;
								return;
							}
							if (opcode == 2701) {
								component = Static6.getComponent(intStack[--isp]);
								if (component.objId != -1) {
									intStack[isp++] = component.objCount;
									return;
								}
								intStack[isp++] = 0;
								return;
							}
							if (opcode == 2702) {
								int1 = intStack[--isp];
								local3254 = (SubInterface) Static329.subInterfaces.get(int1);
								if (local3254 != null) {
									intStack[isp++] = 1;
									return;
								}
								intStack[isp++] = 0;
								return;
							}
							if (opcode == 2703) {
								component = Static6.getComponent(intStack[--isp]);
								if (component.createdComponent == null) {
									intStack[isp++] = 0;
									return;
								}
								int3 = component.createdComponent.length;
								for (local25  = 0; local25  < component.createdComponent.length; local25 ++) {
									if (component.createdComponent[local25 ] == null) {
										int3 = local25 ;
										break;
									}
								}
								intStack[isp++] = int3;
								return;
							}
							if (opcode == 2704 || opcode == 2705) {
								isp -= 2;
								int1 = intStack[isp];
								int3 = intStack[isp + 1];
								local3356 = (SubInterface) Static329.subInterfaces.get(int1);
								if (local3356 != null && local3356.id == int3) {
									intStack[isp++] = 1;
									return;
								} else {
									intStack[isp++] = 0;
									return;
								}
							}
						} else if (opcode < 2900) {
							component = Static6.getComponent(intStack[--isp]);
							if (opcode == 2800) {
								intStack[isp++] = Static45.getServerActiveProperties(component).getTargetMask();
								return;
							}
							if (opcode == 2801) {
								int3 = intStack[--isp];
								int3--;
								if (component.ops != null && int3 < component.ops.length && component.ops[int3] != null) {
									stringStack[anInt3652++] = component.ops[int3];
									return;
								}
								stringStack[anInt3652++] = "";
								return;
							}
							if (opcode == 2802) {
								if (component.opBase == null) {
									stringStack[anInt3652++] = "";
									return;
								}
								stringStack[anInt3652++] = component.opBase;
								return;
							}
						} else {
							@Pc(3493) String local3493;
							if (opcode < 3200) {
								if (opcode == 3100) {
									local3493 = stringStack[--anInt3652];
									Static207.add(local3493);
									return;
								}
								if (opcode == 3101) {
									isp -= 2;
									Static337.animate(Static17.self, intStack[isp + 1], intStack[isp]);
									return;
								}
								if (opcode == 3103) {
									Static82.closeWidget();
									return;
								}
								if (opcode == 3104) {
									local3493 = stringStack[--anInt3652];
									int3 = 0;
									if (Static132.isInt(local3493)) {
										int3 = Static146.parseInt(local3493);
									}
									Static91.method1960(Static363.aClass145_239);
									Static257.outboundBuffer.p4(int3);
									return;
								}
								if (opcode == 3105) {
									local3493 = stringStack[--anInt3652];
									Static91.method1960(Static49.aClass145_44);
									Static257.outboundBuffer.p1(local3493.length() + 1);
									Static257.outboundBuffer.pjstr(local3493);
									return;
								}
								if (opcode == 3106) {
									local3493 = stringStack[--anInt3652];
									Static91.method1960(Static221.aClass145_164);
									Static257.outboundBuffer.p1(local3493.length() + 1);
									Static257.outboundBuffer.pjstr(local3493);
									return;
								}
								if (opcode == 3107) {
									int1 = intStack[--isp];
									local1101 = stringStack[--anInt3652];
									Static341.clickPlayerOption(int1, local1101);
									return;
								}
								if (opcode == 3108) {
									isp -= 3;
									int1 = intStack[isp];
									int3 = intStack[isp + 1];
									local25  = intStack[isp + 2];
									local35 = Static6.getComponent(local25 );
									Static380.dragPickUp(int1, int3, local35);
									return;
								}
								if (opcode == 3109) {
									isp -= 2;
									int1 = intStack[isp];
									int3 = intStack[isp + 1];
									local210 = secondary ? secondaryActiveComponent : primaryActiveComponent;
									Static380.dragPickUp(int1, int3, local210);
									return;
								}
								if (opcode == 3110) {
									int objID = intStack[--isp];
									Static91.method1960(Static190.aClass145_138);
									Static257.outboundBuffer.p2(objID);
									return;
								}
								if (opcode == 3111) {
									isp -= 2;
									int1 = intStack[isp];
									int3 = intStack[isp + 1];
									local3356 = (SubInterface) Static329.subInterfaces.get((long) int1);
									if (local3356 != null) {
										Static276.method4654(true, local3356, local3356.id != int3);
									}
									Static382.method6364(int1, 3, true, int3);
									return;
								}
								if (opcode == 3112) {
									isp--;
									int1 = intStack[isp];
									local3254 = (SubInterface) Static329.subInterfaces.get((long) int1);
									if (local3254 != null && local3254.type == 3) {
										Static276.method4654(true, local3254, true);
									}
									return;
								}
							} else if (opcode < 3300) {
								if (opcode == 3200) {
									isp -= 3;
									Static29.play(intStack[isp + 2], intStack[isp], intStack[isp + 1], 255);
									return;
								}
								if (opcode == 3201) {
									Static138.playMusic(50, 255, intStack[--isp]);
									return;
								}
								if (opcode == 3202) {
									isp -= 2;
									Static175.playJingle(intStack[isp], intStack[isp + 1], 255);
									return;
								}
								if (opcode == 3203) {
									isp -= 4;
									Static29.play(intStack[isp + 2], intStack[isp], intStack[isp + 1], intStack[isp + 3]);
									return;
								}
								if (opcode == 3204) {
									isp -= 3;
									Static138.playMusic(intStack[isp + 2], intStack[isp + 1], intStack[isp]);
									return;
								}
								if (opcode == 3205) {
									isp -= 3;
									Static175.playJingle(intStack[isp], intStack[isp + 1], intStack[isp + 2]);
									return;
								}
							} else if (opcode < 3400) {
								if (opcode == 3300) {
									intStack[isp++] = Static114.loop;
									return;
								}
								if (opcode == 3301) {
									isp -= 2;
									int1 = intStack[isp];
									int3 = intStack[isp + 1];
									intStack[isp++] = Inventory.getItemType(false, int1, int3);
									return;
								}
								if (opcode == 3302) {
									isp -= 2;
									int1 = intStack[isp];
									int3 = intStack[isp + 1];
									intStack[isp++] = Static243.getItemCount(int3, false, int1);
									return;
								}
								if (opcode == 3303) {
									isp -= 2;
									int1 = intStack[isp];
									int3 = intStack[isp + 1];
									intStack[isp++] = Inventory.getSlotTotal(int1, false, int3);
									return;
								}
								if (opcode == 3304) {
									int1 = intStack[--isp];
									intStack[isp++] = Static178.aClass51_3.get(int1).anInt2488;
									return;
								}
								if (opcode == 3305) {
									int1 = intStack[--isp];
									intStack[isp++] = Static268.boostedLevels[int1];
									return;
								}
								if (opcode == 3306) {
									int1 = intStack[--isp];
									intStack[isp++] = Static325.baseLevels[int1];
									return;
								}
								if (opcode == 3307) {
									int1 = intStack[--isp];
									intStack[isp++] = Static80.experience[int1];
									return;
								}
								if (opcode == 3308) {
									@Pc(4136) byte local4136 = Static17.self.aByte78;
									int3 = (Static17.self.xFine >> 7) + Static164.originX;
									local25  = (Static17.self.zFine >> 7) + Static148.originZ;
									intStack[isp++] = (local4136 << 28) + (int3 << 14) + local25 ;
									return;
								}
								if (opcode == 3309) {
									int1 = intStack[--isp];
									intStack[isp++] = int1 >> 14 & 0x3FFF;
									return;
								}
								if (opcode == 3310) {
									int1 = intStack[--isp];
									intStack[isp++] = int1 >> 28;
									return;
								}
								if (opcode == 3311) {
									int1 = intStack[--isp];
									intStack[isp++] = int1 & 0x3FFF;
									return;
								}
								if (opcode == 3312) {
									intStack[isp++] = Static206.mapMembers ? 1 : 0;
									return;
								}
								if (opcode == 3313) {
									isp -= 2;
									int1 = intStack[isp];
									int3 = intStack[isp + 1];
									intStack[isp++] = Inventory.getItemType(true, int1, int3);
									return;
								}
								if (opcode == 3314) {
									isp -= 2;
									int1 = intStack[isp];
									int3 = intStack[isp + 1];
									intStack[isp++] = Static243.getItemCount(int3, true, int1);
									return;
								}
								if (opcode == 3315) {
									isp -= 2;
									int1 = intStack[isp];
									int3 = intStack[isp + 1];
									intStack[isp++] = Inventory.getSlotTotal(int1, true, int3);
									return;
								}
								if (opcode == 3316) {
									if (Static4.staffModLevel >= 2) {
										intStack[isp++] = Static4.staffModLevel;
										return;
									}
									intStack[isp++] = 0;
									return;
								}
								if (opcode == 3317) {
									intStack[isp++] = Static151.anInt2773;
									return;
								}
								if (opcode == 3318) {
									intStack[isp++] = client.worldID;
									return;
								}
								if (opcode == 3321) {
									intStack[isp++] = Static96.anInt2002;
									return;
								}
								if (opcode == 3322) {
									intStack[isp++] = Static41.anInt1048;
									return;
								}
								if (opcode == 3323) {
									if (Static210.blackmarks >= 5 && Static210.blackmarks <= 9) {
										intStack[isp++] = 1;
										return;
									}
									intStack[isp++] = 0;
									return;
								}
								if (opcode == 3324) {
									if (Static210.blackmarks >= 5 && Static210.blackmarks <= 9) {
										intStack[isp++] = Static210.blackmarks;
										return;
									}
									intStack[isp++] = 0;
									return;
								}
								if (opcode == 3325) {
									intStack[isp++] = Static381.playerMemeber ? 1 : 0;
									return;
								}
								if (opcode == 3326) {
									intStack[isp++] = Static17.self.combatLevel;
									return;
								}
								if (opcode == 3327) {
									intStack[isp++] = Static17.self.appearance.female ? 1 : 0;
									return;
								}
								if (opcode == 3328) {
									intStack[isp++] = Static56.playerUnderage && !Static101.aBoolean159 ? 1 : 0;
									return;
								}
								if (opcode == 3329) {
									intStack[isp++] = Static109.enabled ? 1 : 0;
									return;
								}
								if (opcode == 3330) {
									int1 = intStack[--isp];
									intStack[isp++] = Static154.method2784(int1);
									return;
								}
								if (opcode == 3331) {
									isp -= 2;
									int1 = intStack[isp];
									int3 = intStack[isp + 1];
									intStack[isp++] = Inventory.getTotalParam(int3, false, int1);
									return;
								}
								if (opcode == 3332) {
									isp -= 2;
									int1 = intStack[isp];
									int3 = intStack[isp + 1];
									intStack[isp++] = Inventory.getTotalParam(int3, true, int1);
									return;
								}
								if (opcode == 3333) {
									intStack[isp++] = Static202.anInt3714;
									return;
								}
								if (opcode == 3335) {
									intStack[isp++] = client.language;
									return;
								}
								if (opcode == 3336) {
									isp -= 4;
									int1 = intStack[isp];
									int3 = intStack[isp + 1];
									local25  = intStack[isp + 2];
									local353 = intStack[isp + 3];
									int1 += int3 << 14;
									int1 += local25  << 28;
									int1 += local353;
									intStack[isp++] = int1;
									return;
								}
								if (opcode == 3337) {
									intStack[isp++] = Static165.anInt3149;
									return;
								}
								if (opcode == 3338) {
									intStack[isp++] = Static374.method6272();
									return;
								}
							} else if (opcode < 3500) {
								@Pc(4761) EnumType local4761;
								if (opcode == 3400) {
									isp -= 2;
									int1 = intStack[isp];
									int3 = intStack[isp + 1];
									local4761 = EnumTypeList.enumTypeList.get(int1);
									stringStack[anInt3652++] = local4761.getString(int3);
									return;
								}
								if (opcode == 3408) {
									isp -= 4;
									int1 = intStack[isp];
									int3 = intStack[isp + 1];
									local25  = intStack[isp + 2];
									local353 = intStack[isp + 3];
									@Pc(4807) EnumType local4807 = EnumTypeList.enumTypeList.get(local25 );
									if (local4807.keyType == int1 && local4807.valueType == int3) {
										if (int3 == 115) {
											stringStack[anInt3652++] = local4807.getString(local353);
											return;
										}
										intStack[isp++] = local4807.getInt(local353);
										return;
									}
									throw new RuntimeException("C3408-1");
								}
								if (opcode == 3409) {
									isp -= 3;
									int1 = intStack[isp];
									int3 = intStack[isp + 1];
									local25  = intStack[isp + 2];
									if (int3 == -1) {
										throw new RuntimeException("C3409-2");
									}
									@Pc(4883) EnumType local4883 = EnumTypeList.enumTypeList.get(int3);
									if (local4883.valueType != int1) {
										throw new RuntimeException("C3409-1");
									}
									intStack[isp++] = local4883.containsValue(local25 ) ? 1 : 0;
									return;
								}
								if (opcode == 3410) {
									int1 = intStack[--isp];
									local1101 = stringStack[--anInt3652];
									if (int1 == -1) {
										throw new RuntimeException("C3410-2");
									}
									local4761 = EnumTypeList.enumTypeList.get(int1);
									if (local4761.valueType != 's') {
										throw new RuntimeException("C3410-1");
									}
									intStack[isp++] = local4761.containsValue(local1101) ? 1 : 0;
									return;
								}
								if (opcode == 3411) {
									int1 = intStack[--isp];
									@Pc(4981) EnumType local4981 = EnumTypeList.enumTypeList.get(int1);
									intStack[isp++] = local4981.table.method83();
									return;
								}
							} else if (opcode < 3700) {
								if (opcode == 3600) {
									if (Static235.anInt3363 == 0) {
										intStack[isp++] = -2;
										return;
									}
									if (Static235.anInt3363 == 1) {
										intStack[isp++] = -1;
										return;
									}
									intStack[isp++] = Static384.anInt7178;
									return;
								}
								if (opcode == 3601) {
									int1 = intStack[--isp];
									if (Static235.anInt3363 == 2 && int1 < Static384.anInt7178) {
										stringStack[anInt3652++] = Static17.aStringArray43[int1];
										if (Static319.aStringArray54[int1] != null) {
											stringStack[anInt3652++] = Static319.aStringArray54[int1];
											return;
										}
										stringStack[anInt3652++] = "";
										return;
									}
									stringStack[anInt3652++] = "";
									stringStack[anInt3652++] = "";
									return;
								}
								if (opcode == 3602) {
									int1 = intStack[--isp];
									if (Static235.anInt3363 == 2 && int1 < Static384.anInt7178) {
										intStack[isp++] = Static265.anIntArray333[int1];
										return;
									}
									intStack[isp++] = 0;
									return;
								}
								if (opcode == 3603) {
									int1 = intStack[--isp];
									if (Static235.anInt3363 == 2 && int1 < Static384.anInt7178) {
										intStack[isp++] = Static114.anIntArray148[int1];
										return;
									}
									intStack[isp++] = 0;
									return;
								}
								if (opcode == 3604) {
									local3493 = stringStack[--anInt3652];
									int3 = intStack[--isp];
									Static171.method3255(int3, local3493);
									return;
								}
								if (opcode == 3605) {
									local3493 = stringStack[--anInt3652];
									Static61.method1669(local3493);
									return;
								}
								if (opcode == 3606) {
									local3493 = stringStack[--anInt3652];
									Static34.method4827(local3493);
									return;
								}
								if (opcode == 3607) {
									local3493 = stringStack[--anInt3652];
									Static309.method5278(local3493, false);
									return;
								}
								if (opcode == 3608) {
									local3493 = stringStack[--anInt3652];
									Static258.method4403(local3493);
									return;
								}
								if (opcode == 3609) {
									local3493 = stringStack[--anInt3652];
									if (local3493.startsWith("<img=0>") || local3493.startsWith("<img=1>")) {
										local3493 = local3493.substring(7);
									}
									intStack[isp++] = Static391.method6450(local3493) ? 1 : 0;
									return;
								}
								if (opcode == 3610) {
									int1 = intStack[--isp];
									if (Static235.anInt3363 == 2 && int1 < Static384.anInt7178) {
										stringStack[anInt3652++] = Static209.aStringArray32[int1];
										return;
									}
									stringStack[anInt3652++] = "";
									return;
								}
								if (opcode == 3611) {
									if (Static389.aString53 != null) {
										stringStack[anInt3652++] = Static330.method5656(Static389.aString53);
										return;
									}
									stringStack[anInt3652++] = "";
									return;
								}
								if (opcode == 3612) {
									if (Static389.aString53 != null) {
										intStack[isp++] = Static186.anInt3730;
										return;
									}
									intStack[isp++] = 0;
									return;
								}
								if (opcode == 3613) {
									int1 = intStack[--isp];
									if (Static389.aString53 != null && int1 < Static186.anInt3730) {
										stringStack[anInt3652++] = Static47.aClass214Array1[int1].aString58;
										return;
									}
									stringStack[anInt3652++] = "";
									return;
								}
								if (opcode == 3614) {
									int1 = intStack[--isp];
									if (Static389.aString53 != null && int1 < Static186.anInt3730) {
										intStack[isp++] = Static47.aClass214Array1[int1].anInt5978;
										return;
									}
									intStack[isp++] = 0;
									return;
								}
								if (opcode == 3615) {
									int1 = intStack[--isp];
									if (Static389.aString53 != null && int1 < Static186.anInt3730) {
										intStack[isp++] = Static47.aClass214Array1[int1].aByte65;
										return;
									}
									intStack[isp++] = 0;
									return;
								}
								if (opcode == 3616) {
									intStack[isp++] = Static265.aByte54;
									return;
								}
								if (opcode == 3617) {
									local3493 = stringStack[--anInt3652];
									Static163.method3101(local3493);
									return;
								}
								if (opcode == 3618) {
									intStack[isp++] = Static119.aByte25;
									return;
								}
								if (opcode == 3619) {
									local3493 = stringStack[--anInt3652];
									Static115.method3840(local3493);
									return;
								}
								if (opcode == 3620) {
									Static290.method5025();
									return;
								}
								if (opcode == 3621) {
									if (Static235.anInt3363 == 0) {
										intStack[isp++] = -1;
										return;
									}
									intStack[isp++] = Static301.anInt5700;
									return;
								}
								if (opcode == 3622) {
									int1 = intStack[--isp];
									if (Static235.anInt3363 != 0 && int1 < Static301.anInt5700) {
										stringStack[anInt3652++] = Static226.aStringArray35[int1];
										if (Static371.aStringArray66[int1] != null) {
											stringStack[anInt3652++] = Static371.aStringArray66[int1];
											return;
										}
										stringStack[anInt3652++] = "";
										return;
									}
									stringStack[anInt3652++] = "";
									stringStack[anInt3652++] = "";
									return;
								}
								if (opcode == 3623) {
									local3493 = stringStack[--anInt3652];
									if (local3493.startsWith("<img=0>") || local3493.startsWith("<img=1>")) {
										local3493 = local3493.substring(7);
									}
									intStack[isp++] = Static105.method2208(local3493) ? 1 : 0;
									return;
								}
								if (opcode == 3624) {
									int1 = intStack[--isp];
									if (Static47.aClass214Array1 != null && int1 < Static186.anInt3730 && Static47.aClass214Array1[int1].aString60.equalsIgnoreCase(Static17.self.aString41)) {
										intStack[isp++] = 1;
										return;
									}
									intStack[isp++] = 0;
									return;
								}
								if (opcode == 3625) {
									if (Static169.aString25 != null) {
										stringStack[anInt3652++] = Static169.aString25;
										return;
									}
									stringStack[anInt3652++] = "";
									return;
								}
								if (opcode == 3626) {
									int1 = intStack[--isp];
									if (Static389.aString53 != null && int1 < Static186.anInt3730) {
										stringStack[anInt3652++] = Static47.aClass214Array1[int1].aString61;
										return;
									}
									stringStack[anInt3652++] = "";
									return;
								}
								if (opcode == 3627) {
									int1 = intStack[--isp];
									if (Static235.anInt3363 == 2 && int1 >= 0 && int1 < Static384.anInt7178) {
										intStack[isp++] = Static302.aBooleanArray13[int1] ? 1 : 0;
										return;
									}
									intStack[isp++] = 0;
									return;
								}
								if (opcode == 3628) {
									local3493 = stringStack[--anInt3652];
									if (local3493.startsWith("<img=0>") || local3493.startsWith("<img=1>")) {
										local3493 = local3493.substring(7);
									}
									intStack[isp++] = Static117.method2372(local3493);
									return;
								}
								if (opcode == 3629) {
									intStack[isp++] = Static320.anInt6007;
									return;
								}
								if (opcode == 3630) {
									local3493 = stringStack[--anInt3652];
									Static309.method5278(local3493, true);
									return;
								}
								if (opcode == 3631) {
									int1 = intStack[--isp];
									intStack[isp++] = Static196.aBooleanArray31[int1] ? 1 : 0;
									return;
								}
								if (opcode == 3632) {
									int1 = intStack[--isp];
									if (Static389.aString53 != null && int1 < Static186.anInt3730) {
										stringStack[anInt3652++] = Static47.aClass214Array1[int1].aString60;
										return;
									}
									stringStack[anInt3652++] = "";
									return;
								}
								if (opcode == 3633) {
									int1 = intStack[--isp];
									if (Static235.anInt3363 != 0 && int1 < Static301.anInt5700) {
										stringStack[anInt3652++] = Static234.aStringArray39[int1];
										return;
									}
									stringStack[anInt3652++] = "";
									return;
								}
							} else if (opcode < 4000) {
								if (opcode == 3903) {
									int1 = intStack[--isp];
									intStack[isp++] = Static290.aClass187Array1[int1].method4682();
									return;
								}
								if (opcode == 3904) {
									int1 = intStack[--isp];
									intStack[isp++] = Static290.aClass187Array1[int1].anInt5105;
									return;
								}
								if (opcode == 3905) {
									int1 = intStack[--isp];
									intStack[isp++] = Static290.aClass187Array1[int1].anInt5108;
									return;
								}
								if (opcode == 3906) {
									int1 = intStack[--isp];
									intStack[isp++] = Static290.aClass187Array1[int1].anInt5111;
									return;
								}
								if (opcode == 3907) {
									int1 = intStack[--isp];
									intStack[isp++] = Static290.aClass187Array1[int1].anInt5106;
									return;
								}
								if (opcode == 3908) {
									int1 = intStack[--isp];
									intStack[isp++] = Static290.aClass187Array1[int1].anInt5114;
									return;
								}
								if (opcode == 3910) {
									int1 = intStack[--isp];
									int3 = Static290.aClass187Array1[int1].method4678();
									intStack[isp++] = int3 == 0 ? 1 : 0;
									return;
								}
								if (opcode == 3911) {
									int1 = intStack[--isp];
									int3 = Static290.aClass187Array1[int1].method4678();
									intStack[isp++] = int3 == 2 ? 1 : 0;
									return;
								}
								if (opcode == 3912) {
									int1 = intStack[--isp];
									int3 = Static290.aClass187Array1[int1].method4678();
									intStack[isp++] = int3 == 5 ? 1 : 0;
									return;
								}
								if (opcode == 3913) {
									int1 = intStack[--isp];
									int3 = Static290.aClass187Array1[int1].method4678();
									intStack[isp++] = int3 == 1 ? 1 : 0;
									return;
								}
							} else if (opcode < 4100) {
								if (opcode == 4000) {
									isp -= 2;
									int1 = intStack[isp];
									int3 = intStack[isp + 1];
									intStack[isp++] = int1 + int3;
									return;
								}
								if (opcode == 4001) {
									isp -= 2;
									int1 = intStack[isp];
									int3 = intStack[isp + 1];
									intStack[isp++] = int1 - int3;
									return;
								}
								if (opcode == 4002) {
									isp -= 2;
									int1 = intStack[isp];
									int3 = intStack[isp + 1];
									intStack[isp++] = int1 * int3;
									return;
								}
								if (opcode == 4003) {
									isp -= 2;
									int1 = intStack[isp];
									int3 = intStack[isp + 1];
									intStack[isp++] = int1 / int3;
									return;
								}
								if (opcode == 4004) {
									int1 = intStack[--isp];
									intStack[isp++] = (int) (Math.random() * (double) int1);
									return;
								}
								if (opcode == 4005) {
									int1 = intStack[--isp];
									intStack[isp++] = (int) (Math.random() * (double) (int1 + 1));
									return;
								}
								if (opcode == 4006) {
									isp -= 5;
									int1 = intStack[isp];
									int3 = intStack[isp + 1];
									local25  = intStack[isp + 2];
									local353 = intStack[isp + 3];
									local2194 = intStack[isp + 4];
									intStack[isp++] = int1 + (int3 - int1) * (local2194 - local25 ) / (local353 - local25 );
									return;
								}
								@Pc(6491) long local6491;
								@Pc(6484) long local6484;
								if (opcode == 4007) {
									isp -= 2;
									local6484 = intStack[isp];
									local6491 = intStack[isp + 1];
									intStack[isp++] = (int) (local6484 + local6484 * local6491 / 100L);
									return;
								}
								if (opcode == 4008) {
									isp -= 2;
									int1 = intStack[isp];
									int3 = intStack[isp + 1];
									intStack[isp++] = int1 | 0x1 << int3;
									return;
								}
								if (opcode == 4009) {
									isp -= 2;
									int1 = intStack[isp];
									int3 = intStack[isp + 1];
									intStack[isp++] = int1 & -(0x1 << int3) - 1;
									return;
								}
								if (opcode == 4010) {
									isp -= 2;
									int1 = intStack[isp];
									int3 = intStack[isp + 1];
									intStack[isp++] = (int1 & 0x1 << int3) == 0 ? 0 : 1;
									return;
								}
								if (opcode == 4011) {
									isp -= 2;
									int1 = intStack[isp];
									int3 = intStack[isp + 1];
									intStack[isp++] = int1 % int3;
									return;
								}
								if (opcode == 4012) {
									isp -= 2;
									int1 = intStack[isp];
									int3 = intStack[isp + 1];
									if (int1 == 0) {
										intStack[isp++] = 0;
										return;
									}
									intStack[isp++] = (int) Math.pow((double) int1, (double) int3);
									return;
								}
								if (opcode == 4013) {
									isp -= 2;
									int1 = intStack[isp];
									int3 = intStack[isp + 1];
									if (int1 == 0) {
										intStack[isp++] = 0;
										return;
									}
									if (int3 == 0) {
										intStack[isp++] = Integer.MAX_VALUE;
										return;
									}
									intStack[isp++] = (int) Math.pow((double) int1, 1.0D / (double) int3);
									return;
								}
								if (opcode == 4014) {
									isp -= 2;
									int1 = intStack[isp];
									int3 = intStack[isp + 1];
									intStack[isp++] = int1 & int3;
									return;
								}
								if (opcode == 4015) {
									isp -= 2;
									int1 = intStack[isp];
									int3 = intStack[isp + 1];
									intStack[isp++] = int1 | int3;
									return;
								}
								if (opcode == 4016) {
									isp -= 2;
									int1 = intStack[isp];
									int3 = intStack[isp + 1];
									intStack[isp++] = int1 < int3 ? int1 : int3;
									return;
								}
								if (opcode == 4017) {
									isp -= 2;
									int1 = intStack[isp];
									int3 = intStack[isp + 1];
									intStack[isp++] = int1 > int3 ? int1 : int3;
									return;
								}
								if (opcode == 4018) {
									isp -= 3;
									local6484 = intStack[isp];
									local6491 = intStack[isp + 1];
									@Pc(6872) long local6872 = (long) intStack[isp + 2];
									intStack[isp++] = (int) (local6484 * local6872 / local6491);
									return;
								}
							} else if (opcode < 4200) {
								if (opcode == 4100) {
									local3493 = stringStack[--anInt3652];
									int3 = intStack[--isp];
									stringStack[anInt3652++] = local3493 + int3;
									return;
								}
								if (opcode == 4101) {
									anInt3652 -= 2;
									local3493 = stringStack[anInt3652];
									local1101 = stringStack[anInt3652 + 1];
									stringStack[anInt3652++] = local3493 + local1101;
									return;
								}
								if (opcode == 4102) {
									local3493 = stringStack[--anInt3652];
									int3 = intStack[--isp];
									stringStack[anInt3652++] = local3493 + Static320.method5461(int3);
									return;
								}
								if (opcode == 4103) {
									local3493 = stringStack[--anInt3652];
									stringStack[anInt3652++] = local3493.toLowerCase();
									return;
								}
								if (opcode == 4104) {
									stringStack[anInt3652++] = method3593(intStack[--isp]);
									return;
								}
								if (opcode == 4105) {
									anInt3652 -= 2;
									local3493 = stringStack[anInt3652];
									local1101 = stringStack[anInt3652 + 1];
									if (Static17.self.appearance != null && Static17.self.appearance.female) {
										stringStack[anInt3652++] = local1101;
										return;
									}
									stringStack[anInt3652++] = local3493;
									return;
								}
								if (opcode == 4106) {
									int1 = intStack[--isp];
									stringStack[anInt3652++] = Integer.toString(int1);
									return;
								}
								if (opcode == 4107) {
									anInt3652 -= 2;
									intStack[isp++] = Static233.compare(stringStack[anInt3652 + 1], stringStack[anInt3652], client.language);
									return;
								}
								@Pc(7155) Class239 local7155;
								if (opcode == 4108) {
									local3493 = stringStack[--anInt3652];
									isp -= 2;
									int3 = intStack[isp];
									local25  = intStack[isp + 1];
									local7155 = Static101.method2165(local25 , client.js5Archive13);
									intStack[isp++] = local7155.method6122(local3493, int3, Static124.aSpriteArray27);
									return;
								}
								if (opcode == 4109) {
									local3493 = stringStack[--anInt3652];
									isp -= 2;
									int3 = intStack[isp];
									local25  = intStack[isp + 1];
									local7155 = Static101.method2165(local25 , client.js5Archive13);
									intStack[isp++] = local7155.method6123(Static124.aSpriteArray27, local3493, int3);
									return;
								}
								if (opcode == 4110) {
									anInt3652 -= 2;
									local3493 = stringStack[anInt3652];
									local1101 = stringStack[anInt3652 + 1];
									if (intStack[--isp] == 1) {
										stringStack[anInt3652++] = local3493;
										return;
									}
									stringStack[anInt3652++] = local1101;
									return;
								}
								if (opcode == 4111) {
									local3493 = stringStack[--anInt3652];
									stringStack[anInt3652++] = Static153.method2772(local3493);
									return;
								}
								if (opcode == 4112) {
									local3493 = stringStack[--anInt3652];
									int3 = intStack[--isp];
									if (int3 == -1) {
										throw new RuntimeException("null char");
									}
									stringStack[anInt3652++] = local3493 + (char) int3;
									return;
								}
								if (opcode == 4113) {
									int1 = intStack[--isp];
									intStack[isp++] = Static146.method2697((char) int1) ? 1 : 0;
									return;
								}
								if (opcode == 4114) {
									int1 = intStack[--isp];
									intStack[isp++] = Static89.method1939((char) int1) ? 1 : 0;
									return;
								}
								if (opcode == 4115) {
									int1 = intStack[--isp];
									intStack[isp++] = Static35.method1103((char) int1) ? 1 : 0;
									return;
								}
								if (opcode == 4116) {
									int1 = intStack[--isp];
									intStack[isp++] = Static328.method5597((char) int1) ? 1 : 0;
									return;
								}
								if (opcode == 4117) {
									local3493 = stringStack[--anInt3652];
									if (local3493 != null) {
										intStack[isp++] = local3493.length();
										return;
									}
									intStack[isp++] = 0;
									return;
								}
								if (opcode == 4118) {
									local3493 = stringStack[--anInt3652];
									isp -= 2;
									int3 = intStack[isp];
									local25  = intStack[isp + 1];
									stringStack[anInt3652++] = local3493.substring(int3, local25 );
									return;
								}
								if (opcode == 4119) {
									local3493 = stringStack[--anInt3652];
									@Pc(7518) StringBuffer local7518 = new StringBuffer(local3493.length());
									@Pc(7520) boolean local7520 = false;
									for (local353 = 0; local353 < local3493.length(); local353++) {
										@Pc(7527) char local7527 = local3493.charAt(local353);
										if (local7527 == '<') {
											local7520 = true;
										} else if (local7527 == '>') {
											local7520 = false;
										} else if (!local7520) {
											local7518.append(local7527);
										}
									}
									stringStack[anInt3652++] = local7518.toString();
									return;
								}
								if (opcode == 4120) {
									local3493 = stringStack[--anInt3652];
									isp -= 2;
									int3 = intStack[isp];
									local25  = intStack[isp + 1];
									intStack[isp++] = local3493.indexOf(int3, local25 );
									return;
								}
								if (opcode == 4121) {
									anInt3652 -= 2;
									local3493 = stringStack[anInt3652];
									local1101 = stringStack[anInt3652 + 1];
									local25  = intStack[--isp];
									intStack[isp++] = local3493.indexOf(local1101, local25 );
									return;
								}
								if (opcode == 4122) {
									int1 = intStack[--isp];
									intStack[isp++] = Character.toLowerCase((char) int1);
									return;
								}
								if (opcode == 4123) {
									int1 = intStack[--isp];
									intStack[isp++] = Character.toUpperCase((char) int1);
									return;
								}
								if (opcode == 4124) {
									local412 = intStack[--isp] != 0;
									int3 = intStack[--isp];
									stringStack[anInt3652++] = Static172.method3288((long) int3, client.language, local412, 0);
									return;
								}
								if (opcode == 4125) {
									local3493 = stringStack[--anInt3652];
									int3 = intStack[--isp];
									@Pc(7741) Class239 local7741 = Static101.method2165(int3, client.js5Archive13);
									intStack[isp++] = local7741.method6126(Static124.aSpriteArray27, local3493);
									return;
								}
							} else {
								@Pc(8083) Class240 local8083;
								if (opcode < 4300) {
									if (opcode == 4200) {
										int1 = intStack[--isp];
										stringStack[anInt3652++] = ObjTypeList.objTypeList.get(int1).name;
										return;
									}
									@Pc(7803) ObjType local7803;
									if (opcode == 4201) {
										isp -= 2;
										int1 = intStack[isp];
										int3 = intStack[isp + 1];
										local7803 = ObjTypeList.objTypeList.get(int1);
										if (int3 >= 1 && int3 <= 5 && local7803.ops[int3 - 1] != null) {
											stringStack[anInt3652++] = local7803.ops[int3 - 1];
											return;
										}
										stringStack[anInt3652++] = "";
										return;
									}
									if (opcode == 4202) {
										isp -= 2;
										int1 = intStack[isp];
										int3 = intStack[isp + 1];
										local7803 = ObjTypeList.objTypeList.get(int1);
										if (int3 >= 1 && int3 <= 5 && local7803.iOps[int3 - 1] != null) {
											stringStack[anInt3652++] = local7803.iOps[int3 - 1];
											return;
										}
										stringStack[anInt3652++] = "";
										return;
									}
									if (opcode == 4203) {
										int1 = intStack[--isp];
										intStack[isp++] = ObjTypeList.objTypeList.get(int1).cost;
										return;
									}
									if (opcode == 4204) {
										int1 = intStack[--isp];
										intStack[isp++] = ObjTypeList.objTypeList.get(int1).stackable == 1 ? 1 : 0;
										return;
									}
									@Pc(7966) ObjType local7966;
									if (opcode == 4205) {
										int1 = intStack[--isp];
										local7966 = ObjTypeList.objTypeList.get(int1);
										if (local7966.certificateTemplate == -1 && local7966.certificateLink >= 0) {
											intStack[isp++] = local7966.certificateLink;
											return;
										}
										intStack[isp++] = int1;
										return;
									}
									if (opcode == 4206) {
										int1 = intStack[--isp];
										local7966 = ObjTypeList.objTypeList.get(int1);
										if (local7966.certificateTemplate >= 0 && local7966.certificateLink >= 0) {
											intStack[isp++] = local7966.certificateLink;
											return;
										}
										intStack[isp++] = int1;
										return;
									}
									if (opcode == 4207) {
										int1 = intStack[--isp];
										intStack[isp++] = ObjTypeList.objTypeList.get(int1).members ? 1 : 0;
										return;
									}
									if (opcode == 4208) {
										isp -= 2;
										int1 = intStack[isp];
										int3 = intStack[isp + 1];
										local8083 = Static363.aClass18_2.method565(int3);
										if (local8083.method6262()) {
											stringStack[anInt3652++] = ObjTypeList.objTypeList.get(int1).method4772(local8083.aString69, int3);
											return;
										}
										intStack[isp++] = ObjTypeList.objTypeList.get(int1).method4771(local8083.anInt7023, int3);
										return;
									}
									if (opcode == 4209) {
										isp -= 2;
										int1 = intStack[isp];
										int3 = intStack[isp + 1] - 1;
										local7803 = ObjTypeList.objTypeList.get(int1);
										if (local7803.anInt5244 == int3) {
											intStack[isp++] = local7803.anInt5204;
											return;
										}
										if (local7803.anInt5221 == int3) {
											intStack[isp++] = local7803.anInt5240;
											return;
										}
										intStack[isp++] = -1;
										return;
									}
									if (opcode == 4210) {
										local3493 = stringStack[--anInt3652];
										int3 = intStack[--isp];
										Static131.findObjs(local3493, int3 == 1);
										intStack[isp++] = Static328.size;
										return;
									}
									if (opcode == 4211) {
										if (Static177.results != null && Static12.index < Static328.size) {
											intStack[isp++] = Static177.results[Static12.index++] & 0xFFFF;
											return;
										}
										intStack[isp++] = -1;
										return;
									}
									if (opcode == 4212) {
										Static12.index = 0;
										return;
									}
								} else if (opcode < 4400) {
									if (opcode == 4300) {
										isp -= 2;
										int1 = intStack[isp];
										int3 = intStack[isp + 1];
										local8083 = Static363.aClass18_2.method565(int3);
										if (local8083.method6262()) {
											stringStack[anInt3652++] = NpcTypeList.npcTypeList.get(int1).method2090(local8083.aString69, int3);
											return;
										}
										intStack[isp++] = NpcTypeList.npcTypeList.get(int1).method2098(local8083.anInt7023, int3);
										return;
									}
								} else if (opcode < 4500) {
									if (opcode == 4400) {
										isp -= 2;
										int1 = intStack[isp];
										int3 = intStack[isp + 1];
										local8083 = Static363.aClass18_2.method565(int3);
										if (local8083.method6262()) {
											stringStack[anInt3652++] = LocTypeList.locTypeList.method5162(int1).method1520(local8083.aString69, int3);
											return;
										}
										intStack[isp++] = LocTypeList.locTypeList.method5162(int1).method1532(local8083.anInt7023, int3);
										return;
									}
								} else if (opcode < 4600) {
									if (opcode == 4500) {
										isp -= 2;
										int1 = intStack[isp];
										int3 = intStack[isp + 1];
										local8083 = Static363.aClass18_2.method565(int3);
										if (local8083.method6262()) {
											stringStack[anInt3652++] = Static367.aClass132_1.method3470(int1).method1346(int3, local8083.aString69);
											return;
										}
										intStack[isp++] = Static367.aClass132_1.method3470(int1).method1343(int3, local8083.anInt7023);
										return;
									}
								} else if (opcode < 4700 && opcode == 4600) {
									int1 = intStack[--isp];
									@Pc(8467) BasType local8467 = Static59.aClass7_1.method245(int1);
									if (local8467.anIntArray426 != null && local8467.anIntArray426.length > 0) {
										local25  = 0;
										local353 = local8467.anIntArray427[0];
										for (local2194 = 1; local2194 < local8467.anIntArray426.length; local2194++) {
											if (local8467.anIntArray427[local2194] > local353) {
												local25  = local2194;
												local353 = local8467.anIntArray427[local2194];
											}
										}
										intStack[isp++] = local8467.anIntArray426[local25 ];
										return;
									}
									intStack[isp++] = local8467.readySeqID;
									return;
								}
							}
						}
					}
				}
			} else {
				if (opcode >= 2000) {
					opcode -= 1000;
					component = Static6.getComponent(intStack[--isp]);
				} else {
					component = secondary ? secondaryActiveComponent : primaryActiveComponent;
				}
				if (opcode == 1000) {
					isp -= 4;
					component.anInt4241 = intStack[isp];
					component.anInt4249 = intStack[isp + 1];
					int3 = intStack[isp + 2];
					if (int3 < 0) {
						int3 = 0;
					} else if (int3 > 5) {
						int3 = 5;
					}
					local25  = intStack[isp + 3];
					if (local25  < 0) {
						local25  = 0;
					} else if (local25  > 5) {
						local25  = 5;
					}
					component.aByte45 = (byte) int3;
					component.aByte46 = (byte) local25 ;
					Static200.redraw(component);
					Static58.method1624(component);
					if (component.createdComponentID == -1) {
						Static139.method2585(component.id);
					}
					return;
				}
				if (opcode == 1001) {
					isp -= 4;
					component.anInt4294 = intStack[isp];
					component.anInt4289 = intStack[isp + 1];
					component.anInt4251 = 0;
					component.anInt4269 = 0;
					int3 = intStack[isp + 2];
					if (int3 < 0) {
						int3 = 0;
					} else if (int3 > 4) {
						int3 = 4;
					}
					local25  = intStack[isp + 3];
					if (local25  < 0) {
						local25  = 0;
					} else if (local25  > 4) {
						local25  = 4;
					}
					component.widthMode = (byte) int3;
					component.heightMode = (byte) local25 ;
					Static200.redraw(component);
					Static58.method1624(component);
					if (component.type == 0) {
						Static85.method1911(false, component);
					}
					return;
				}
				if (opcode == 1003) {
					@Pc(620) boolean local620 = intStack[--isp] == 1;
					if (component.hidden != local620) {
						component.hidden = local620;
						Static200.redraw(component);
					}
					if (component.createdComponentID == -1) {
						Static216.method3777(component.id);
					}
					return;
				}
				if (opcode == 1004) {
					isp -= 2;
					component.anInt4243 = intStack[isp];
					component.anInt4291 = intStack[isp + 1];
					Static200.redraw(component);
					Static58.method1624(component);
					if (component.type == 0) {
						Static85.method1911(false, component);
					}
					return;
				}
				if (opcode == 1005) {
					component.noClickThrough = intStack[--isp] == 1;
					return;
				}
			}
		}
		throw new IllegalStateException();
	}

	@OriginalMember(owner = "client!lk", name = "a", descriptor = "(Lclient!bc;II)V")
	public static void method3595(@OriginalArg(0) Class16 arg0, @OriginalArg(1) int arg1, @OriginalArg(2) int arg2) {
		@Pc(5) ClientScript local5 = Static185.method3398(arg2, arg0, arg1);
		if (local5 == null) {
			return;
		}
		intLocals = new int[local5.intLocals];
		stringLocals = new String[local5.stringLocals];
		if (local5.aClass16_4 == Static383.aClass16_9 || local5.aClass16_4 == Static309.aClass16_7 || local5.aClass16_4 == Static3.aClass16_1) {
			@Pc(30) int local30 = 0;
			@Pc(32) int local32 = 0;
			if (Static227.aComponent_11 != null) {
				local30 = Static227.aComponent_11.x;
				local32 = Static227.aComponent_11.y;
			}
			intLocals[0] = Static226.aClass119_1.method3303() - local30;
			intLocals[1] = Static226.aClass119_1.method3312() - local32;
		}
		run(local5, 200000);
	}

	@OriginalMember(owner = "client!lk", name = "c", descriptor = "(I)V")
	private static void method3596(@OriginalArg(0) int arg0) {
		@Pc(3) Component local3 = Static6.getComponent(arg0);
		if (local3 == null) {
			return;
		}
		@Pc(9) int local9 = arg0 >>> 16;
		@Pc(13) Component[] local13 = Static113.aComponentArrayArray2[local9];
		if (local13 == null) {
			@Pc(19) Component[] local19 = Static26.aComponentArrayArray1[local9];
			@Pc(22) int local22 = local19.length;
			local13 = Static113.aComponentArrayArray2[local9] = new Component[local22];
			Static403.method4608(local19, 0, local13, 0, local19.length);
		}
		@Pc(38) int local38;
		for (local38 = 0; local38 < local13.length && local13[local38] != local3; local38++) {
		}
		if (local38 >= local13.length) {
			return;
		}
		Static403.method4608(local13, 0, local13, 1, local38);
		local13[0] = local3;
	}

	@OriginalMember(owner = "client!lk", name = "d", descriptor = "(I)V")
	public static void method3597(@OriginalArg(0) int arg0) {
		if (arg0 == -1 || !Static222.method3915(arg0)) {
			return;
		}
		@Pc(12) Component[] local12 = Static26.aComponentArrayArray1[arg0];
		for (@Pc(14) int local14 = 0; local14 < local12.length; local14++) {
			@Pc(19) Component local19 = local12[local14];
			if (local19.anObjectArray22 != null) {
				@Pc(26) HookRequest local26 = new HookRequest();
				local26.source = local19;
				local26.arguments = local19.anObjectArray22;
				method3592(local26, 2000000);
			}
		}
	}

	@OriginalMember(owner = "client!lk", name = "b", descriptor = "(IZ)V")
	private static void method3598(@OriginalArg(0) int opcode, @OriginalArg(1) boolean arg1) {
		@Pc(157) int local157;
		@Pc(1292) boolean local1292;
		@Pc(192) int local192;
		@Pc(95) int local95;
		@Pc(89) int local89;
		@Pc(81) String local81;
		@Pc(75) String local75;
		if (opcode < 5100) {
			if (opcode == 5000) {
				intStack[isp++] = Static20.anInt545;
				return;
			}
			if (opcode == 5001) {
				isp -= 3;
				Static20.anInt545 = intStack[isp];
				Static154.aClass170_2 = Static335.method5694(intStack[isp + 1]);
				if (Static154.aClass170_2 == null) {
					Static154.aClass170_2 = Static329.aClass170_4;
				}
				Static331.anInt6206 = intStack[isp + 2];
				Static91.method1960(Static32.aClass145_32);
				Static257.outboundBuffer.p1(Static20.anInt545);
				Static257.outboundBuffer.p1(Static154.aClass170_2.anInt4690);
				Static257.outboundBuffer.p1(Static331.anInt6206);
				return;
			}
			if (opcode == 5002) {
				anInt3652 -= 2;
				local75 = stringStack[anInt3652];
				local81 = stringStack[anInt3652 + 1];
				isp -= 2;
				local89 = intStack[isp];
				local95 = intStack[isp + 1];
				if (local81 == null) {
					local81 = "";
				}
				if (local81.length() > 80) {
					local81 = local81.substring(0, 80);
				}
				Static91.method1960(Static327.aClass145_222);
				Static257.outboundBuffer.p1(Static27.method872(local75) + Static27.method872(local81) + 2);
				Static257.outboundBuffer.pjstr(local75);
				Static257.outboundBuffer.p1(local89 - 1);
				Static257.outboundBuffer.p1(local95);
				Static257.outboundBuffer.pjstr(local81);
				return;
			}
			if (opcode == 5003) {
				local157 = intStack[--isp];
				local81 = null;
				if (local157 < 100) {
					local81 = Static88.aStringArray13[local157];
				}
				if (local81 == null) {
					local81 = "";
				}
				stringStack[anInt3652++] = local81;
				return;
			}
			if (opcode == 5004) {
				local157 = intStack[--isp];
				local192 = -1;
				if (local157 < 100 && Static88.aStringArray13[local157] != null) {
					local192 = Static291.anIntArray373[local157];
				}
				intStack[isp++] = local192;
				return;
			}
			if (opcode == 5005) {
				if (Static154.aClass170_2 == null) {
					intStack[isp++] = -1;
					return;
				}
				intStack[isp++] = Static154.aClass170_2.anInt4690;
				return;
			}
			if (opcode == 5008 || opcode == 5021) {
				local75 = stringStack[--anInt3652];
				method3591(local75, opcode);
				return;
			}
			if (opcode == 5009) {
				anInt3652 -= 2;
				local75 = stringStack[anInt3652];
				local81 = stringStack[anInt3652 + 1];
				if (Static4.staffModLevel != 0 || (!Static56.playerUnderage || Static101.aBoolean159) && !Static109.enabled) {
					Static91.method1960(Static382.aClass145_261);
					Static257.outboundBuffer.p1(0);
					local89 = Static257.outboundBuffer.offset;
					Static257.outboundBuffer.pjstr(local75);
					Static120.method2379(local81, Static257.outboundBuffer);
					Static257.outboundBuffer.psize1(Static257.outboundBuffer.offset - local89);
					return;
				}
				return;
			}
			if (opcode == 5010) {
				local157 = intStack[--isp];
				local81 = null;
				if (local157 < 100) {
					local81 = Static42.aStringArray59[local157];
				}
				if (local81 == null) {
					local81 = "";
				}
				stringStack[anInt3652++] = local81;
				return;
			}
			if (opcode == 5011) {
				local157 = intStack[--isp];
				local81 = null;
				if (local157 < 100) {
					local81 = Static135.aStringArray20[local157];
				}
				if (local81 == null) {
					local81 = "";
				}
				stringStack[anInt3652++] = local81;
				return;
			}
			if (opcode == 5012) {
				local157 = intStack[--isp];
				local192 = -1;
				if (local157 < 100) {
					local192 = Static132.phraseIDs[local157];
				}
				intStack[isp++] = local192;
				return;
			}
			if (opcode == 5015) {
				if (Static17.self == null || Static17.self.username == null) {
					local75 = Static374.username;
				} else {
					local75 = Static17.self.method4071();
				}
				stringStack[anInt3652++] = local75;
				return;
			}
			if (opcode == 5016) {
				intStack[isp++] = Static331.anInt6206;
				return;
			}
			if (opcode == 5017) {
				intStack[isp++] = Static226.anInt4016;
				return;
			}
			if (opcode == 5018) {
				local157 = intStack[--isp];
				local192 = 0;
				if (local157 < 100 && Static88.aStringArray13[local157] != null) {
					local192 = Static291.anIntArray373[local157];
				}
				intStack[isp++] = local192;
				return;
			}
			if (opcode == 5019) {
				local157 = intStack[--isp];
				local81 = null;
				if (local157 < 100) {
					local81 = Static169.aStringArray26[local157];
				}
				if (local81 == null) {
					local81 = "";
				}
				stringStack[anInt3652++] = local81;
				return;
			}
			if (opcode == 5020) {
				if (Static17.self == null || Static17.self.username == null) {
					local75 = Static374.username;
				} else {
					local75 = Static17.self.method4069();
				}
				stringStack[anInt3652++] = local75;
				return;
			}
			if (opcode == 5050) {
				local157 = intStack[--isp];
				stringStack[anInt3652++] = Static138.get(local157).description;
				return;
			}
			@Pc(584) QuickChatCategoryType local584;
			if (opcode == 5051) {
				local157 = intStack[--isp];
				local584 = Static138.get(local157);
				if (local584.subcategories == null) {
					intStack[isp++] = 0;
					return;
				}
				intStack[isp++] = local584.subcategories.length;
				return;
			}
			if (opcode == 5052) {
				isp -= 2;
				local157 = intStack[isp];
				local192 = intStack[isp + 1];
				@Pc(628) QuickChatCategoryType local628 = Static138.get(local157);
				local95 = local628.subcategories[local192];
				intStack[isp++] = local95;
				return;
			}
			if (opcode == 5053) {
				local157 = intStack[--isp];
				local584 = Static138.get(local157);
				if (local584.phrases == null) {
					intStack[isp++] = 0;
					return;
				}
				intStack[isp++] = local584.phrases.length;
				return;
			}
			if (opcode == 5054) {
				isp -= 2;
				local157 = intStack[isp];
				local192 = intStack[isp + 1];
				intStack[isp++] = Static138.get(local157).phrases[local192];
				return;
			}
			if (opcode == 5055) {
				local157 = intStack[--isp];
				stringStack[anInt3652++] = Static146.get(local157).getText();
				return;
			}
			if (opcode == 5056) {
				local157 = intStack[--isp];
				@Pc(750) QuickChatPhraseType type = Static146.get(local157);
				if (type.automaticResponse == null) {
					intStack[isp++] = 0;
					return;
				}
				intStack[isp++] = type.automaticResponse.length;
				return;
			}
			if (opcode == 5057) {
				isp -= 2;
				local157 = intStack[isp];
				local192 = intStack[isp + 1];
				intStack[isp++] = Static146.get(local157).automaticResponse[local192];
				return;
			}
			if (opcode == 5058) {
				activePhrase = new QuickChatPhrase();
				activePhrase.id = intStack[--isp];
				activePhrase.type = Static146.get(activePhrase.id);
				activePhrase.values = new int[activePhrase.type.getDynamicCommandCount()];
				return;
			}
			if (opcode == 5059) {
				Static91.method1960(Static369.aClass145_248);
				Static257.outboundBuffer.p1(0);
				local157 = Static257.outboundBuffer.offset;
				Static257.outboundBuffer.p1(0);
				Static257.outboundBuffer.p2(activePhrase.id);
				activePhrase.type.encodeMessage(Static257.outboundBuffer, activePhrase.values);
				Static257.outboundBuffer.psize1(Static257.outboundBuffer.offset - local157);
				return;
			}
			if (opcode == 5060) {
				local75 = stringStack[--anInt3652];
				Static91.method1960(Static325.aClass145_220);
				Static257.outboundBuffer.p1(0);
				local192 = Static257.outboundBuffer.offset;
				Static257.outboundBuffer.pjstr(local75);
				Static257.outboundBuffer.p2(activePhrase.id);
				activePhrase.type.encodeMessage(Static257.outboundBuffer, activePhrase.values);
				Static257.outboundBuffer.psize1(Static257.outboundBuffer.offset - local192);
				return;
			}
			if (opcode == 5061) {
				Static91.method1960(Static369.aClass145_248);
				Static257.outboundBuffer.p1(0);
				local157 = Static257.outboundBuffer.offset;
				Static257.outboundBuffer.p1(1);
				Static257.outboundBuffer.p2(activePhrase.id);
				activePhrase.type.encodeMessage(Static257.outboundBuffer, activePhrase.values);
				Static257.outboundBuffer.psize1(Static257.outboundBuffer.offset - local157);
				return;
			}
			if (opcode == 5062) {
				isp -= 2;
				local157 = intStack[isp];
				local192 = intStack[isp + 1];
				intStack[isp++] = Static138.get(local157).subcategoryShortcuts[local192];
				return;
			}
			if (opcode == 5063) {
				isp -= 2;
				local157 = intStack[isp];
				local192 = intStack[isp + 1];
				intStack[isp++] = Static138.get(local157).phraseShortcuts[local192];
				return;
			}
			if (opcode == 5064) {
				isp -= 2;
				local157 = intStack[isp];
				local192 = intStack[isp + 1];
				if (local192 == -1) {
					intStack[isp++] = -1;
					return;
				}
				intStack[isp++] = Static138.get(local157).getSubcategoryByShortcut((char) local192);
				return;
			}
			if (opcode == 5065) {
				isp -= 2;
				local157 = intStack[isp];
				local192 = intStack[isp + 1];
				if (local192 == -1) {
					intStack[isp++] = -1;
					return;
				}
				intStack[isp++] = Static138.get(local157).getPhraseByShortcut((char) local192);
				return;
			}
			if (opcode == 5066) {
				local157 = intStack[--isp];
				intStack[isp++] = Static146.get(local157).getDynamicCommandCount();
				return;
			}
			if (opcode == 5067) {
				isp -= 2;
				local157 = intStack[isp];
				local192 = intStack[isp + 1];
				local89 = Static146.get(local157).getDynamicCommand(local192);
				intStack[isp++] = local89;
				return;
			}
			if (opcode == 5068) {
				isp -= 2;
				local157 = intStack[isp];
				local192 = intStack[isp + 1];
				activePhrase.values[local157] = local192;
				return;
			}
			if (opcode == 5069) {
				isp -= 2;
				local157 = intStack[isp];
				local192 = intStack[isp + 1];
				activePhrase.values[local157] = local192;
				return;
			}
			if (opcode == 5070) {
				isp -= 3;
				local157 = intStack[isp];
				local192 = intStack[isp + 1];
				local89 = intStack[isp + 2];
				@Pc(1245) QuickChatPhraseType local1245 = Static146.get(local157);
				if (local1245.getDynamicCommand(local192) != 0) {
					throw new RuntimeException("bad command");
				}
				intStack[isp++] = local1245.getDynamicCommandParam(local192, local89);
				return;
			}
			if (opcode == 5071) {
				local75 = stringStack[--anInt3652];
				local1292 = intStack[--isp] == 1;
				Static272.findQuickChatPhrases(local1292, local75);
				intStack[isp++] = Static328.size;
				return;
			}
			if (opcode == 5072) {
				if (Static177.results != null && Static12.index < Static328.size) {
					intStack[isp++] = Static177.results[Static12.index++] & 0xFFFF;
					return;
				}
				intStack[isp++] = -1;
				return;
			}
			if (opcode == 5073) {
				Static12.index = 0;
				return;
			}
		} else if (opcode < 5200) {
			if (opcode == 5100) {
				if (Static174.aClass123_2.method3342(86)) {
					intStack[isp++] = 1;
					return;
				}
				intStack[isp++] = 0;
				return;
			}
			if (opcode == 5101) {
				if (Static174.aClass123_2.method3342(82)) {
					intStack[isp++] = 1;
					return;
				}
				intStack[isp++] = 0;
				return;
			}
			if (opcode == 5102) {
				if (Static174.aClass123_2.method3342(81)) {
					intStack[isp++] = 1;
					return;
				}
				intStack[isp++] = 0;
				return;
			}
		} else {
			@Pc(1903) boolean local1903;
			if (opcode < 5300) {
				if (opcode == 5200) {
					Static211.setTargetZoom(intStack[--isp]);
					return;
				}
				if (opcode == 5201) {
					intStack[isp++] = Static333.getTargetZoom();
					return;
				}
				if (opcode == 5205) {
					Static258.method4401(false, -1, intStack[--isp], -1);
					return;
				}
				@Pc(1492) Map local1492;
				if (opcode == 5206) {
					int coordinate = intStack[--isp];
					local1492 = Static344.getContainingSource(coordinate >> 14 & 0x3FFF, coordinate & 0x3FFF);
					if (local1492 == null) {
						intStack[isp++] = -1;
						return;
					}
					intStack[isp++] = local1492.id;
					return;
				}
				@Pc(1525) Map local1525;
				if (opcode == 5207) {
					local1525 = Static344.get(intStack[--isp]);
					if (local1525 != null && local1525.name != null) {
						stringStack[anInt3652++] = local1525.name;
						return;
					}
					stringStack[anInt3652++] = "";
					return;
				}
				if (opcode == 5208) {
					intStack[isp++] = Static44.anInt1105;
					intStack[isp++] = Static368.anInt6930;
					return;
				}
				if (opcode == 5209) {
					intStack[isp++] = Static151.anInt2772 + Static344.anInt6057;
					intStack[isp++] = Static96.anInt2003 + Static344.anInt6055;
					return;
				}
				if (opcode == 5210) {
					local157 = intStack[--isp];
					local1492 = Static344.get(local157);
					if (local1492 == null) {
						intStack[isp++] = 0;
						intStack[isp++] = 0;
						return;
					}
					intStack[isp++] = local1492.origin >> 14 & 0x3FFF;
					intStack[isp++] = local1492.origin & 0x3FFF;
					return;
				}
				if (opcode == 5211) {
					local157 = intStack[--isp];
					local1492 = Static344.get(local157);
					if (local1492 == null) {
						intStack[isp++] = 0;
						intStack[isp++] = 0;
						return;
					}
					intStack[isp++] = local1492.anInt6868 - local1492.anInt6870;
					intStack[isp++] = local1492.anInt6874 - local1492.anInt6863;
					return;
				}
				@Pc(1715) Class2_Sub20 local1715;
				if (opcode == 5212) {
					local1715 = Static209.method3712();
					if (local1715 == null) {
						intStack[isp++] = -1;
						intStack[isp++] = -1;
						return;
					}
					intStack[isp++] = local1715.anInt2947;
					local192 = local1715.anInt2946 << 28 | local1715.anInt2949 + Static344.anInt6057 << 14 | local1715.anInt2952 + Static344.anInt6055;
					intStack[isp++] = local192;
					return;
				}
				if (opcode == 5213) {
					local1715 = Static115.method3828();
					if (local1715 == null) {
						intStack[isp++] = -1;
						intStack[isp++] = -1;
						return;
					}
					intStack[isp++] = local1715.anInt2947;
					local192 = local1715.anInt2946 << 28 | local1715.anInt2949 + Static344.anInt6057 << 14 | local1715.anInt2952 + Static344.anInt6055;
					intStack[isp++] = local192;
					return;
				}
				@Pc(1863) boolean local1863;
				if (opcode == 5214) {
					local157 = intStack[--isp];
					local1492 = Static378.method6311();
					if (local1492 != null) {
						local1863 = local1492.method6144(local157 >> 28 & 0x3, anIntArray215, local157 >> 14 & 0x3FFF, local157 & 0x3FFF);
						if (local1863) {
							Static15.method394(anIntArray215[2], anIntArray215[1]);
						}
					}
					return;
				}
				if (opcode == 5215) {
					isp -= 2;
					local157 = intStack[isp];
					local192 = intStack[isp + 1];
					@Pc(1901) Class246 local1901 = Static344.method5515(local157 >> 14 & 0x3FFF, local157 & 0x3FFF);
					local1903 = false;
					for (@Pc(1908) Map local1908 = (Map) local1901.head(); local1908 != null; local1908 = (Map) local1901.next()) {
						if (local1908.id == local192) {
							local1903 = true;
							break;
						}
					}
					if (local1903) {
						intStack[isp++] = 1;
						return;
					}
					intStack[isp++] = 0;
					return;
				}
				if (opcode == 5218) {
					local157 = intStack[--isp];
					local1492 = Static344.get(local157);
					if (local1492 == null) {
						intStack[isp++] = -1;
						return;
					}
					intStack[isp++] = local1492.anInt6875;
					return;
				}
				if (opcode == 5220) {
					intStack[isp++] = Static355.loadPercentage == 100 ? 1 : 0;
					return;
				}
				if (opcode == 5221) {
					local157 = intStack[--isp];
					Static15.method394(local157 & 0x3FFF, local157 >> 14 & 0x3FFF);
					return;
				}
				if (opcode == 5222) {
					local1525 = Static378.method6311();
					if (local1525 != null) {
						local1292 = local1525.method6148(anIntArray215, Static96.anInt2003 + Static344.anInt6055, Static151.anInt2772 + Static344.anInt6057);
						if (local1292) {
							intStack[isp++] = anIntArray215[1];
							intStack[isp++] = anIntArray215[2];
							return;
						}
						intStack[isp++] = -1;
						intStack[isp++] = -1;
						return;
					}
					intStack[isp++] = -1;
					intStack[isp++] = -1;
					return;
				}
				if (opcode == 5223) {
					isp -= 2;
					local157 = intStack[isp];
					local192 = intStack[isp + 1];
					Static258.method4401(false, local192 >> 14 & 0x3FFF, local157, local192 & 0x3FFF);
					return;
				}
				if (opcode == 5224) {
					local157 = intStack[--isp];
					local1492 = Static378.method6311();
					if (local1492 != null) {
						local1863 = local1492.method6144(local157 >> 28 & 0x3, anIntArray215, local157 >> 14 & 0x3FFF, local157 & 0x3FFF);
						if (local1863) {
							intStack[isp++] = anIntArray215[1];
							intStack[isp++] = anIntArray215[2];
							return;
						}
						intStack[isp++] = -1;
						intStack[isp++] = -1;
						return;
					}
					intStack[isp++] = -1;
					intStack[isp++] = -1;
					return;
				}
				if (opcode == 5225) {
					local157 = intStack[--isp];
					local1492 = Static378.method6311();
					if (local1492 != null) {
						local1863 = local1492.method6148(anIntArray215, local157 & 0x3FFF, local157 >> 14 & 0x3FFF);
						if (local1863) {
							intStack[isp++] = anIntArray215[1];
							intStack[isp++] = anIntArray215[2];
							return;
						}
						intStack[isp++] = -1;
						intStack[isp++] = -1;
						return;
					}
					intStack[isp++] = -1;
					intStack[isp++] = -1;
					return;
				}
				if (opcode == 5226) {
					Static225.method3954(intStack[--isp]);
					return;
				}
				if (opcode == 5227) {
					isp -= 2;
					local157 = intStack[isp];
					local192 = intStack[isp + 1];
					Static258.method4401(true, local192 >> 14 & 0x3FFF, local157, local192 & 0x3FFF);
					return;
				}
				if (opcode == 5228) {
					Static175.aBoolean225 = intStack[--isp] == 1;
					return;
				}
				if (opcode == 5229) {
					intStack[isp++] = Static175.aBoolean225 ? 1 : 0;
					return;
				}
				if (opcode == 5230) {
					local157 = intStack[--isp];
					Static232.method4026(local157);
					return;
				}
				@Pc(2421) Node local2421;
				if (opcode == 5231) {
					isp -= 2;
					local157 = intStack[isp];
					local1292 = intStack[isp + 1] == 1;
					if (Static141.aClass4_58 != null) {
						local2421 = Static141.aClass4_58.get(local157);
						if (local2421 != null && !local1292) {
							local2421.unlink();
							return;
						}
						if (local2421 == null && local1292) {
							local2421 = new Node();
							Static141.aClass4_58.put(local2421, local157);
						}
					}
					return;
				}
				@Pc(2463) Node local2463;
				if (opcode == 5232) {
					local157 = intStack[--isp];
					if (Static141.aClass4_58 != null) {
						local2463 = Static141.aClass4_58.get(local157);
						intStack[isp++] = local2463 == null ? 0 : 1;
						return;
					}
					intStack[isp++] = 0;
					return;
				}
				if (opcode == 5233) {
					isp -= 2;
					local157 = intStack[isp];
					local1292 = intStack[isp + 1] == 1;
					if (Static220.aClass4_83 != null) {
						local2421 = Static220.aClass4_83.get(local157);
						if (local2421 != null && !local1292) {
							local2421.unlink();
							return;
						}
						if (local2421 == null && local1292) {
							local2421 = new Node();
							Static220.aClass4_83.put(local2421, local157);
						}
					}
					return;
				}
				if (opcode == 5234) {
					local157 = intStack[--isp];
					if (Static220.aClass4_83 != null) {
						local2463 = Static220.aClass4_83.get((long) local157);
						intStack[isp++] = local2463 == null ? 0 : 1;
						return;
					}
					intStack[isp++] = 0;
					return;
				}
				if (opcode == 5235) {
					intStack[isp++] = Static344.aMap_3 == null ? -1 : Static344.aMap_3.id;
					return;
				}
			} else {
				@Pc(2717) int local2717;
				if (opcode < 5400) {
					if (opcode == 5300) {
						isp -= 2;
						local157 = intStack[isp];
						local192 = intStack[isp + 1];
						Static143.setWindowMode(false, 3, local192, local157);
						intStack[isp++] = GameShell.fullScreenFrame == null ? 0 : 1;
						return;
					}
					if (opcode == 5301) {
						if (GameShell.fullScreenFrame != null) {
							Static143.setWindowMode(false, Static218.preferences.anInt4894, -1, -1);
						}
						return;
					}
					if (opcode == 5302) {
						@Pc(2654) Class169[] local2654 = Static63.method1703();
						intStack[isp++] = local2654.length;
						return;
					}
					if (opcode == 5303) {
						local157 = intStack[--isp];
						@Pc(2678) Class169[] local2678 = Static63.method1703();
						intStack[isp++] = local2678[local157].anInt4685;
						intStack[isp++] = local2678[local157].anInt4684;
						return;
					}
					if (opcode == 5305) {
						local157 = Static218.preferences.anInt4884;
						local192 = Static218.preferences.anInt4893;
						local89 = -1;
						@Pc(2715) Class169[] local2715 = Static63.method1703();
						for (local2717 = 0; local2717 < local2715.length; local2717++) {
							@Pc(2722) Class169 local2722 = local2715[local2717];
							if (local2722.anInt4685 == local157 && local2722.anInt4684 == local192) {
								local89 = local2717;
								break;
							}
						}
						intStack[isp++] = local89;
						return;
					}
					if (opcode == 5306) {
						intStack[isp++] = Static144.getWindowMode();
						return;
					}
					if (opcode == 5307) {
						local157 = intStack[--isp];
						if (local157 >= 1 && local157 <= 2) {
							Static143.setWindowMode(false, local157, -1, -1);
							return;
						}
						return;
					}
					if (opcode == 5308) {
						intStack[isp++] = Static218.preferences.anInt4894;
						return;
					}
					if (opcode == 5309) {
						local157 = intStack[--isp];
						if (local157 >= 1 && local157 <= 2) {
							Static218.preferences.anInt4894 = local157;
							Static218.preferences.write(GameShell.signlink);
							return;
						}
						return;
					}
				} else {
					@Pc(3230) String local3230;
					if (opcode < 5500) {
						if (opcode == 5400) {
							anInt3652 -= 2;
							local75 = stringStack[anInt3652];
							local81 = stringStack[anInt3652 + 1];
							local89 = intStack[--isp];
							Static91.method1960(Static323.aClass145_219);
							Static257.outboundBuffer.p1(Static27.method872(local75) + Static27.method872(local81) + 1);
							Static257.outboundBuffer.pjstr(local75);
							Static257.outboundBuffer.pjstr(local81);
							Static257.outboundBuffer.p1(local89);
							return;
						}
						if (opcode == 5401) {
							isp -= 2;
							Static257.aShortArray79[intStack[isp]] = (short) Static376.method6284(intStack[isp + 1]);
							ObjTypeList.objTypeList.method2767();
							ObjTypeList.objTypeList.method2760();
							NpcTypeList.npcTypeList.method5571();
							Static98.method2107();
							return;
						}
						if (opcode == 5405) {
							isp -= 2;
							local157 = intStack[isp];
							local192 = intStack[isp + 1];
							if (local157 >= 0 && local157 < 2) {
								Static364.anIntArrayArrayArray13[local157] = new int[local192 << 1][4];
							}
							return;
						}
						if (opcode == 5406) {
							isp -= 7;
							local157 = intStack[isp];
							local192 = intStack[isp + 1] << 1;
							local89 = intStack[isp + 2];
							local95 = intStack[isp + 3];
							local2717 = intStack[isp + 4];
							@Pc(2990) int local2990 = intStack[isp + 5];
							@Pc(2996) int local2996 = intStack[isp + 6];
							if (local157 >= 0 && local157 < 2 && Static364.anIntArrayArrayArray13[local157] != null && local192 >= 0 && local192 < Static364.anIntArrayArrayArray13[local157].length) {
								Static364.anIntArrayArrayArray13[local157][local192] = new int[] { (local89 >> 14 & 0x3FFF) * 128, local95, (local89 & 0x3FFF) * 128, local2996 };
								Static364.anIntArrayArrayArray13[local157][local192 + 1] = new int[] { (local2717 >> 14 & 0x3FFF) * 128, local2990, (local2717 & 0x3FFF) * 128 };
							}
							return;
						}
						if (opcode == 5407) {
							local157 = Static364.anIntArrayArrayArray13[intStack[--isp]].length >> 1;
							intStack[isp++] = local157;
							return;
						}
						if (opcode == 5411) {
							if (GameShell.fullScreenFrame != null) {
								Static143.setWindowMode(false, Static218.preferences.anInt4894, -1, -1);
							}
							if (GameShell.frame != null) {
								Static340.method5764();
								System.exit(0);
								return;
							}
							local75 = Static307.aString57 == null ? Static200.method3649() : Static307.aString57;
							Static280.method4764(local75, GameShell.signlink, Static77.anInt1762 == 1, false);
							return;
						}
						if (opcode == 5419) {
							local75 = "";
							if (Static354.aPrivelegedRequest_5 != null) {
								if (Static354.aPrivelegedRequest_5.result == null) {
									local75 = Static150.method2739(Static354.aPrivelegedRequest_5.anInt992);
								} else {
									local75 = (String) Static354.aPrivelegedRequest_5.result;
								}
							}
							stringStack[anInt3652++] = local75;
							return;
						}
						if (opcode == 5420) {
							intStack[isp++] = Static215.anInt3795 == 3 ? 1 : 0;
							return;
						}
						if (opcode == 5421) {
							if (GameShell.fullScreenFrame != null) {
								Static143.setWindowMode(false, Static218.preferences.anInt4894, -1, -1);
							}
							local75 = stringStack[--anInt3652];
							local1292 = intStack[--isp] == 1;
							local3230 = Static200.method3649() + local75;
							Static280.method4764(local3230, GameShell.signlink, Static77.anInt1762 == 1, local1292);
							return;
						}
						if (opcode == 5422) {
							anInt3652 -= 2;
							local75 = stringStack[anInt3652];
							local81 = stringStack[anInt3652 + 1];
							local89 = intStack[--isp];
							if (local75.length() > 0) {
								if (Static129.aStringArray19 == null) {
									Static129.aStringArray19 = new String[Static342.anIntArray384[client.game.gameTypeID]];
								}
								Static129.aStringArray19[local89] = local75;
							}
							if (local81.length() > 0) {
								if (Static333.aStringArray67 == null) {
									Static333.aStringArray67 = new String[Static342.anIntArray384[client.game.gameTypeID]];
								}
								Static333.aStringArray67[local89] = local81;
							}
							return;
						}
						if (opcode == 5423) {
							System.out.println(stringStack[--anInt3652]);
							return;
						}
						if (opcode == 5424) {
							isp -= 11;
							Static242.anInt4446 = intStack[isp];
							Static260.anInt6717 = intStack[isp + 1];
							Static200.anInt3697 = intStack[isp + 2];
							Static86.anInt1868 = intStack[isp + 3];
							Static186.anInt3731 = intStack[isp + 4];
							Static67.anInt1636 = intStack[isp + 5];
							Static74.anInt1722 = intStack[isp + 6];
							Static363.anInt6804 = intStack[isp + 7];
							Static390.anInt7211 = intStack[isp + 8];
							Static322.anInt6030 = intStack[isp + 9];
							Static381.anInt7118 = intStack[isp + 10];
							client.js5Archive8.isFileReady(Static186.anInt3731);
							client.js5Archive8.isFileReady(Static67.anInt1636);
							client.js5Archive8.isFileReady(Static74.anInt1722);
							client.js5Archive8.isFileReady(Static363.anInt6804);
							client.js5Archive8.isFileReady(Static390.anInt7211);
							Static337.aSprite_17 = null;
							Static110.aSprite_6 = null;
							Static35.aSprite_3 = null;
							Static75.aSprite_5 = null;
							Static138.aSprite_8 = null;
							Static197.aSprite_12 = null;
							Static339.aSprite_18 = null;
							Static211.aSprite = null;
							Static270.aBoolean371 = true;
							return;
						}
						if (opcode == 5425) {
							Static190.method3501();
							Static270.aBoolean371 = false;
							return;
						}
						if (opcode == 5426) {
							isp -= 2;
							Static394.anInt7263 = intStack[isp];
							Static91.anInt1910 = intStack[isp + 1];
							return;
						}
						if (opcode == 5427) {
							isp -= 2;
							Static135.anInt2523 = intStack[isp + 1];
							return;
						}
						if (opcode == 5428) {
							isp -= 2;
							local157 = intStack[isp];
							local192 = intStack[isp + 1];
							intStack[isp++] = Static281.method4784(local192, local157) ? 1 : 0;
							return;
						}
						if (opcode == 5429) {
							Static161.execute(stringStack[--anInt3652], false);
							return;
						}
					} else if (opcode < 5600) {
						if (opcode == 5500) {
							isp -= 4;
							local157 = intStack[isp];
							local192 = intStack[isp + 1];
							local89 = intStack[isp + 2];
							local95 = intStack[isp + 3];
							Static51.method1545((local157 & 0x3FFF) - Static148.originZ, local89, local95, false, local192, (local157 >> 14 & 0x3FFF) - Static164.originX);
							return;
						}
						if (opcode == 5501) {
							isp -= 4;
							local157 = intStack[isp];
							local192 = intStack[isp + 1];
							local89 = intStack[isp + 2];
							local95 = intStack[isp + 3];
							Static33.method4202(local89, local192, (local157 & 0x3FFF) - Static148.originZ, local95, (local157 >> 14 & 0x3FFF) - Static164.originX);
							return;
						}
						if (opcode == 5502) {
							isp -= 6;
							local157 = intStack[isp];
							if (local157 >= 2) {
								throw new RuntimeException();
							}
							Static147.anInt2717 = local157;
							local192 = intStack[isp + 1];
							if (local192 + 1 >= Static364.anIntArrayArrayArray13[Static147.anInt2717].length >> 1) {
								throw new RuntimeException();
							}
							Static295.anInt5567 = local192;
							Static353.anInt6533 = 0;
							Static41.anInt1038 = intStack[isp + 2];
							Static217.anInt3821 = intStack[isp + 3];
							local89 = intStack[isp + 4];
							if (local89 >= 2) {
								throw new RuntimeException();
							}
							Static349.anInt6455 = local89;
							local95 = intStack[isp + 5];
							if (local95 + 1 >= Static364.anIntArrayArrayArray13[Static349.anInt6455].length >> 1) {
								throw new RuntimeException();
							}
							Static343.anInt6398 = local95;
							Static314.anInt5911 = 3;
							return;
						}
						if (opcode == 5503) {
							Static117.method2369();
							return;
						}
						if (opcode == 5504) {
							isp -= 2;
							Static231.method4021(intStack[isp], intStack[isp + 1]);
							return;
						}
						if (opcode == 5505) {
							intStack[isp++] = (int) Static31.aFloat28 >> 3;
							return;
						}
						if (opcode == 5506) {
							intStack[isp++] = (int) Static277.aFloat67 >> 3;
							return;
						}
						if (opcode == 5507) {
							Static350.method5836();
							return;
						}
						if (opcode == 5508) {
							Static121.method2396();
							return;
						}
						if (opcode == 5509) {
							Static352.method5879();
							return;
						}
						if (opcode == 5510) {
							Static26.method862();
							return;
						}
						if (opcode == 5511) {
							local157 = intStack[--isp];
							local192 = local157 >> 14 & 0x3FFF;
							local89 = local157 & 0x3FFF;
							local192 -= Static164.originX;
							if (local192 < 0) {
								local192 = 0;
							} else if (local192 >= Static373.anInt7033) {
								local192 = Static373.anInt7033;
							}
							local89 -= Static148.originZ;
							if (local89 < 0) {
								local89 = 0;
							} else if (local89 >= Static242.anInt4449) {
								local89 = Static242.anInt4449;
							}
							Static394.anInt7264 = local192 * 128 + 64;
							Static348.anInt6448 = local89 * 128 + 64;
							Static314.anInt5911 = 4;
							return;
						}
						if (opcode == 5512) {
							Static351.method5858();
							return;
						}
					} else if (opcode < 5700) {
						if (opcode == 5600) {
							anInt3652 -= 2;
							local75 = stringStack[anInt3652];
							local81 = stringStack[anInt3652 + 1];
							local89 = intStack[--isp];
							if (client.gameState != 10) {
								return;
							}
							if (Static154.anInt2803 == 0 && Static347.step == 0 && Static141.anInt2652 == 0 && Static77.step == 0) {
								Static97.method2087(local81, local75, local89);
								return;
							}
							return;
						}
						if (opcode == 5601) {
							Static319.method5434();
							return;
						}
						if (opcode == 5602) {
							if (Static347.step == 0) {
								Static41.reply = -2;
							}
							return;
						}
						if (opcode == 5603) {
							isp -= 4;
							if (client.gameState != 10) {
								return;
							}
							if (Static154.anInt2803 == 0 && Static347.step == 0 && Static141.anInt2652 == 0 && Static77.step == 0) {
								Static352.method5875(intStack[isp + 3], intStack[isp + 2], intStack[isp], intStack[isp + 1]);
								return;
							}
							return;
						}
						if (opcode == 5604) {
							anInt3652--;
							if (client.gameState != 10) {
								return;
							}
							if (Static154.anInt2803 == 0 && Static347.step == 0 && Static141.anInt2652 == 0 && Static77.step == 0) {
								Static72.method1809(Static80.encode(stringStack[anInt3652]));
								return;
							}
							return;
						}
						if (opcode == 5605) {
							anInt3652 -= 3;
							isp -= 7;
							if (client.gameState != 10) {
								return;
							}
							if (Static154.anInt2803 == 0 && Static347.step == 0 && Static141.anInt2652 == 0 && Static77.step == 0) {
								Static389.method4487(stringStack[anInt3652 + 1], intStack[isp], intStack[isp + 3], intStack[isp + 4] == 1, Static80.encode(stringStack[anInt3652]), intStack[isp + 2], intStack[isp + 1], intStack[isp + 6] == 1, intStack[isp + 5] == 1, stringStack[anInt3652 + 2]);
								return;
							}
							return;
						}
						if (opcode == 5606) {
							if (Static141.anInt2652 == 0) {
								Static318.anInt5968 = -2;
							}
							return;
						}
						if (opcode == 5607) {
							intStack[isp++] = Static41.reply;
							return;
						}
						if (opcode == 5608) {
							intStack[isp++] = Static47.hopTime;
							return;
						}
						if (opcode == 5609) {
							intStack[isp++] = Static318.anInt5968;
							return;
						}
						if (opcode == 5610) {
							for (local157 = 0; local157 < 5; local157++) {
								stringStack[anInt3652++] = Static163.aStringArray23.length > local157 ? Static330.method5656(Static163.aStringArray23[local157]) : "";
							}
							Static163.aStringArray23 = null;
							return;
						}
						if (opcode == 5611) {
							intStack[isp++] = Static360.anInt6186;
							return;
						}
					} else if (opcode < 6100) {
						if (opcode == 6001) {
							local157 = intStack[--isp];
							if (local157 < 1) {
								local157 = 1;
							}
							if (local157 > 4) {
								local157 = 4;
							}
							Static218.preferences.brightness = local157;
							Static347.method5826();
							Static218.preferences.write(GameShell.signlink);
							Static236.sentToServer = false;
							return;
						}
						if (opcode == 6002) {
							Static218.preferences.setAllLevelsVisible(intStack[--isp] == 1);
							Static347.method5826();
							Static226.method3981();
							Static218.preferences.write(GameShell.signlink);
							Static236.sentToServer = false;
							return;
						}
						if (opcode == 6003) {
							Static218.preferences.roofsVisible = intStack[--isp] == 1;
							Static226.method3981();
							Static218.preferences.write(GameShell.signlink);
							Static236.sentToServer = false;
							return;
						}
						if (opcode == 6005) {
							Static218.preferences.groundDecoration = intStack[--isp] == 1;
							Static347.method5826();
							Static218.preferences.write(GameShell.signlink);
							Static236.sentToServer = false;
							return;
						}
						if (opcode == 6006) {
							Static218.preferences.highDetailTextures = intStack[--isp] == 1;
							Static190.aClass19_8.method2874(!Static218.preferences.highDetailTextures);
							Static218.preferences.write(GameShell.signlink);
							Static236.sentToServer = false;
							return;
						}
						if (opcode == 6007) {
							Static218.preferences.manyIdleAnimations = intStack[--isp];
							Static218.preferences.write(GameShell.signlink);
							Static236.sentToServer = false;
							return;
						}
						if (opcode == 6008) {
							Static218.preferences.flickeringEffects = intStack[--isp] == 1;
							Static218.preferences.write(GameShell.signlink);
							Static236.sentToServer = false;
							return;
						}
						if (opcode == 6009) {
							Static218.preferences.groundTextures = intStack[--isp] == 1;
							Static347.method5826();
							Static218.preferences.write(GameShell.signlink);
							Static236.sentToServer = false;
							return;
						}
						if (opcode == 6010) {
							Static218.preferences.characterShadows = intStack[--isp] == 1;
							Static218.preferences.write(GameShell.signlink);
							Static236.sentToServer = false;
							return;
						}
						if (opcode == 6011) {
							int sceneryShadows = intStack[--isp];
							if (sceneryShadows < 0 || sceneryShadows > 2) {
								sceneryShadows = 0;
							}
							Static218.preferences.sceneryShadows = sceneryShadows;
							Static347.method5826();
							Static218.preferences.write(GameShell.signlink);
							Static236.sentToServer = false;
							return;
						}
						if (opcode == 6012) {
							Static218.preferences.highDetailLighting = intStack[--isp] == 1;
							Static174.method1506();
							Static218.preferences.write(GameShell.signlink);
							Static236.sentToServer = false;
							return;
						}
						if (opcode == 6014) {
							Static218.preferences.highDetailWater = intStack[--isp] == 1;
							Static347.method5826();
							Static218.preferences.write(GameShell.signlink);
							Static236.sentToServer = false;
							return;
						}
						if (opcode == 6015) {
							Static218.preferences.fog = intStack[--isp] == 1;
							Static347.method5826();
							Static218.preferences.write(GameShell.signlink);
							Static236.sentToServer = false;
							return;
						}
						if (opcode == 6016) {
							local157 = intStack[--isp];
							if (local157 < 0 || local157 > 2) {
								local157 = 0;
							}
							Static218.preferences.anInt4885 = local157;
							Static353.method5882();
							Static218.preferences.write(GameShell.signlink);
							return;
						}
						if (opcode == 6017) {
							Static218.preferences.stereo = intStack[--isp] == 1;
							Static178.method2342();
							Static218.preferences.write(GameShell.signlink);
							Static236.sentToServer = false;
							return;
						}
						if (opcode == 6018) {
							int volume = intStack[--isp];
							if (volume < 0) {
								volume = 0;
							}
							if (volume > 127) {
								volume = 127;
							}
							Static218.preferences.soundEffectsVolume = volume;
							Static218.preferences.write(GameShell.signlink);
							Static236.sentToServer = false;
							return;
						}
						if (opcode == 6019) {
							int volume = intStack[--isp];
							if (volume < 0) {
								volume = 0;
							}
							if (volume > 255) {
								volume = 255;
							}
							if (volume != Static218.preferences.musicVolume) {
								if (Static218.preferences.musicVolume == 0 && Static171.groupID != -1) {
									Static94.playImmediate(Static171.groupID, client.js5Archive6, volume);
									Static393.jingle = false;
								} else if (volume == 0) {
									Static368.method6191();
									Static393.jingle = false;
								} else {
									Static303.setVolume(volume);
								}
								Static218.preferences.musicVolume = volume;
							}
							Static218.preferences.write(GameShell.signlink);
							Static236.sentToServer = false;
							return;
						}
						if (opcode == 6020) {
							int volume = intStack[--isp];
							if (volume < 0) {
								volume = 0;
							}
							if (volume > 127) {
								volume = 127;
							}
							Static218.preferences.areaSoundsVolume = volume;
							Static218.preferences.write(GameShell.signlink);
							Static236.sentToServer = false;
							return;
						}
						if (opcode == 6021) {
							Static218.preferences.aBoolean366 = intStack[--isp] == 1;
							Static226.method3981();
							return;
						}
						if (opcode == 6023) {
							int particles = intStack[--isp];
							boolean forceDisabled = false;
							if (particles < 0) {
								particles = 0;
							}
							if (particles > 2) {
								particles = 2;
							}
							if (GameShell.maxMemory < 96) {
								particles = 0;
								forceDisabled = true;
							}
							Preferences.setParticles(particles);
							Static218.preferences.write(GameShell.signlink);
							Static236.sentToServer = false;
							intStack[isp++] = forceDisabled ? 0 : 1;
							return;
						}
						if (opcode == 6024) {
							local157 = intStack[--isp];
							if (local157 < 0 || local157 > 2) {
								local157 = 0;
							}
							Static218.preferences.antiAliasingMode = local157;
							Static218.preferences.write(GameShell.signlink);
							return;
						}
						if (opcode == 6025) {
							local157 = intStack[--isp];
							if (local157 < 0 || local157 > Static289.method5018(GameShell.maxMemory)) {
								local157 = 0;
							}
							Static218.preferences.buildArea = local157;
							Static218.preferences.write(GameShell.signlink);
							Static236.sentToServer = false;
							return;
						}
						if (opcode == 6027) {
							local157 = intStack[--isp];
							if (local157 < 0 || local157 > 1) {
								local157 = 0;
							}
							Static284.method4881(local157 == 1);
							return;
						}
						if (opcode == 6028) {
							Static218.preferences.customCursors = intStack[--isp] != 0;
							Static218.preferences.write(GameShell.signlink);
							return;
						}
						if (opcode == 6029) {
							Static218.preferences.manyIdleAnimations = intStack[--isp];
							Static218.preferences.write(GameShell.signlink);
							return;
						}
						if (opcode == 6030) {
							Static218.preferences.aBoolean357 = intStack[--isp] != 0;
							Static218.preferences.write(GameShell.signlink);
							Static347.method5826();
							return;
						}
						if (opcode == 6031) {
							local157 = intStack[--isp];
							if (local157 < 0 || local157 > 3) {
								local157 = 2;
							}
							Static78.method5700(local157);
							return;
						}
						if (opcode == 6032) {
							local157 = intStack[--isp];
							if (local157 < 0 || local157 > 3) {
								local157 = 2;
							}
							Static218.preferences.anInt4878 = local157;
							Static218.preferences.write(GameShell.signlink);
							Static236.sentToServer = false;
							return;
						}
						if (opcode == 6033) {
							local157 = intStack[--isp];
							if (local157 < 0 || local157 > 4) {
								local157 = 2;
							}
							Static218.preferences.anInt4881 = local157;
							Static218.preferences.write(GameShell.signlink);
							return;
						}
						if (opcode == 6035) {
							Static218.preferences.aBoolean365 = intStack[--isp] == 1;
							Static347.method5826();
							Static226.method3981();
							return;
						}
					} else if (opcode < 6200) {
						if (opcode == 6101) {
							intStack[isp++] = Static218.preferences.brightness;
							return;
						}
						if (opcode == 6102) {
							intStack[isp++] = Static218.preferences.method4495(Static77.anInt1762) ? 1 : 0;
							return;
						}
						if (opcode == 6103) {
							intStack[isp++] = Static218.preferences.roofsVisible ? 1 : 0;
							return;
						}
						if (opcode == 6105) {
							intStack[isp++] = Static218.preferences.groundDecoration ? 1 : 0;
							return;
						}
						if (opcode == 6106) {
							intStack[isp++] = Static218.preferences.highDetailTextures ? 1 : 0;
							return;
						}
						if (opcode == 6107) {
							intStack[isp++] = Static218.preferences.manyIdleAnimations;
							return;
						}
						if (opcode == 6108) {
							intStack[isp++] = Static218.preferences.flickeringEffects ? 1 : 0;
							return;
						}
						if (opcode == 6109) {
							intStack[isp++] = Static218.preferences.groundTextures ? 1 : 0;
							return;
						}
						if (opcode == 6110) {
							intStack[isp++] = Static218.preferences.characterShadows ? 1 : 0;
							return;
						}
						if (opcode == 6111) {
							intStack[isp++] = Static218.preferences.sceneryShadows;
							return;
						}
						if (opcode == 6112) {
							intStack[isp++] = Static218.preferences.highDetailLighting ? 1 : 0;
							return;
						}
						if (opcode == 6114) {
							intStack[isp++] = Static218.preferences.highDetailWater ? 1 : 0;
							return;
						}
						if (opcode == 6115) {
							intStack[isp++] = Static218.preferences.fog ? 1 : 0;
							return;
						}
						if (opcode == 6116) {
							intStack[isp++] = Static218.preferences.anInt4885;
							return;
						}
						if (opcode == 6117) {
							intStack[isp++] = Static218.preferences.stereo ? 1 : 0;
							return;
						}
						if (opcode == 6118) {
							intStack[isp++] = Static218.preferences.soundEffectsVolume;
							return;
						}
						if (opcode == 6119) {
							intStack[isp++] = Static218.preferences.musicVolume;
							return;
						}
						if (opcode == 6120) {
							intStack[isp++] = Static218.preferences.areaSoundsVolume;
							return;
						}
						if (opcode == 6121) {
							intStack[isp++] = Static190.aClass19_8.method2894() ? 1 : 0;
							return;
						}
						if (opcode == 6123) {
							intStack[isp++] = Static184.getParticles();
							return;
						}
						if (opcode == 6124) {
							intStack[isp++] = Static218.preferences.antiAliasingMode;
							return;
						}
						if (opcode == 6125) {
							intStack[isp++] = Static218.preferences.buildArea;
							return;
						}
						if (opcode == 6126) {
							intStack[isp++] = Static190.aClass19_8.method2858() ? 1 : 0;
							return;
						}
						if (opcode == 6127) {
							intStack[isp++] = Static218.preferences.aBoolean356 ? 1 : 0;
							return;
						}
						if (opcode == 6128) {
							intStack[isp++] = Static218.preferences.customCursors ? 1 : 0;
							return;
						}
						if (opcode == 6129) {
							intStack[isp++] = Static218.preferences.manyIdleAnimations;
							return;
						}
						if (opcode == 6130) {
							intStack[isp++] = Static218.preferences.aBoolean357 ? 1 : 0;
							return;
						}
						if (opcode == 6131) {
							intStack[isp++] = Static77.anInt1762;
							return;
						}
						if (opcode == 6132) {
							intStack[isp++] = Static218.preferences.anInt4878;
							return;
						}
						if (opcode == 6133) {
							intStack[isp++] = Static215.anInt3795 == 1 || Static215.anInt3795 == 4 ? 1 : 0;
							return;
						}
						if (opcode == 6134) {
							intStack[isp++] = Static289.method5018(GameShell.maxMemory);
							return;
						}
						if (opcode == 6135) {
							intStack[isp++] = Static218.preferences.anInt4881;
							return;
						}
					} else if (opcode < 6300) {
						if (opcode == 6200) {
							isp -= 2;
							Static254.aShort73 = (short) intStack[isp];
							if (Static254.aShort73 <= 0) {
								Static254.aShort73 = 256;
							}
							Static258.aShort78 = (short) intStack[isp + 1];
							if (Static258.aShort78 <= 0) {
								Static258.aShort78 = 205;
							}
							return;
						}
						if (opcode == 6201) {
							isp -= 2;
							Static271.aShort79 = (short) intStack[isp];
							if (Static271.aShort79 <= 0) {
								Static271.aShort79 = 256;
							}
							Static120.aShort36 = (short) intStack[isp + 1];
							if (Static120.aShort36 <= 0) {
								Static120.aShort36 = 320;
							}
							return;
						}
						if (opcode == 6202) {
							isp -= 4;
							Static78.aShort86 = (short) intStack[isp];
							if (Static78.aShort86 <= 0) {
								Static78.aShort86 = 1;
							}
							Static120.aShort35 = (short) intStack[isp + 1];
							if (Static120.aShort35 <= 0) {
								Static120.aShort35 = 32767;
							} else if (Static120.aShort35 < Static78.aShort86) {
								Static120.aShort35 = Static78.aShort86;
							}
							Static353.aShort92 = (short) intStack[isp + 2];
							if (Static353.aShort92 <= 0) {
								Static353.aShort92 = 1;
							}
							Static68.aShort19 = (short) intStack[isp + 3];
							if (Static68.aShort19 <= 0) {
								Static68.aShort19 = 32767;
								return;
							}
							if (Static68.aShort19 < Static353.aShort92) {
								Static68.aShort19 = Static353.aShort92;
							}
							return;
						}
						if (opcode == 6203) {
							Static308.method5272(0, Static20.aComponent_1.height, false, Static20.aComponent_1.width, 0);
							intStack[isp++] = Static155.anInt2808;
							intStack[isp++] = Static180.anInt3408;
							return;
						}
						if (opcode == 6204) {
							intStack[isp++] = Static271.aShort79;
							intStack[isp++] = Static120.aShort36;
							return;
						}
						if (opcode == 6205) {
							intStack[isp++] = Static254.aShort73;
							intStack[isp++] = Static258.aShort78;
							return;
						}
					} else if (opcode < 6400) {
						if (opcode == 6300) {
							intStack[isp++] = (int) (MonotonicClock.currentTimeMillis() / 60000L);
							return;
						}
						if (opcode == 6301) {
							intStack[isp++] = (int) (MonotonicClock.currentTimeMillis() / 86400000L) - 11745;
							return;
						}
						if (opcode == 6302) {
							isp -= 3;
							local157 = intStack[isp];
							local192 = intStack[isp + 1];
							local89 = intStack[isp + 2];
							calendar.clear();
							calendar.set(11, 12);
							calendar.set(local89, local192, local157);
							intStack[isp++] = (int) (calendar.getTime().getTime() / 86400000L) - 11745;
							return;
						}
						if (opcode == 6303) {
							calendar.clear();
							calendar.setTime(new Date(MonotonicClock.currentTimeMillis()));
							intStack[isp++] = calendar.get(1);
							return;
						}
						if (opcode == 6304) {
							local157 = intStack[--isp];
							local1292 = true;
							if (local157 < 0) {
								local1292 = (local157 + 1) % 4 == 0;
							} else if (local157 < 1582) {
								local1292 = local157 % 4 == 0;
							} else if (local157 % 4 != 0) {
								local1292 = false;
							} else if (local157 % 100 != 0) {
								local1292 = true;
							} else if (local157 % 400 != 0) {
								local1292 = false;
							}
							intStack[isp++] = local1292 ? 1 : 0;
							return;
						}
					} else if (opcode < 6500) {
						if (opcode == 6405) {
							intStack[isp++] = Static79.method1870() ? 1 : 0;
							return;
						}
						if (opcode == 6406) {
							intStack[isp++] = Static331.method5659() ? 1 : 0;
							return;
						}
					} else if (opcode < 6600) {
						if (opcode == 6500) {
							if (client.gameState == 10 && Static154.anInt2803 == 0 && Static347.step == 0 && Static141.anInt2652 == 0) {
								intStack[isp++] = Static319.fetch() == -1 ? 0 : 1;
								return;
							}
							intStack[isp++] = 1;
							return;
						}
						@Pc(5979) Country local5979;
						@Pc(5946) World local5946;
						if (opcode == 6501) {
							local5946 = Static19.method559();
							if (local5946 != null) {
								intStack[isp++] = local5946.worldID;
								intStack[isp++] = local5946.flags;
								stringStack[anInt3652++] = local5946.activity;
								local5979 = local5946.getCountry();
								intStack[isp++] = local5979.flag;
								stringStack[anInt3652++] = local5979.name;
								intStack[isp++] = local5946.anInt78;
								intStack[isp++] = local5946.anInt83;
								return;
							}
							intStack[isp++] = -1;
							intStack[isp++] = 0;
							stringStack[anInt3652++] = "";
							intStack[isp++] = 0;
							stringStack[anInt3652++] = "";
							intStack[isp++] = 0;
							intStack[isp++] = 0;
							return;
						}
						if (opcode == 6502) {
							local5946 = Static268.method4547();
							if (local5946 != null) {
								intStack[isp++] = local5946.worldID;
								intStack[isp++] = local5946.flags;
								stringStack[anInt3652++] = local5946.activity;
								local5979 = local5946.getCountry();
								intStack[isp++] = local5979.flag;
								stringStack[anInt3652++] = local5979.name;
								intStack[isp++] = local5946.anInt78;
								intStack[isp++] = local5946.anInt83;
								return;
							}
							intStack[isp++] = -1;
							intStack[isp++] = 0;
							stringStack[anInt3652++] = "";
							intStack[isp++] = 0;
							stringStack[anInt3652++] = "";
							intStack[isp++] = 0;
							intStack[isp++] = 0;
							return;
						}
						if (opcode == 6503) {
							local157 = intStack[--isp];
							if (client.gameState == 10 && Static154.anInt2803 == 0 && Static347.step == 0 && Static141.anInt2652 == 0) {
								intStack[isp++] = Static375.switchWorld(local157) ? 1 : 0;
								return;
							}
							intStack[isp++] = 0;
							return;
						}
						if (opcode == 6504) {
							Static218.preferences.anInt4887 = intStack[--isp];
							Static218.preferences.write(GameShell.signlink);
							return;
						}
						if (opcode == 6505) {
							intStack[isp++] = Static218.preferences.anInt4887;
							return;
						}
						if (opcode == 6506) {
							local157 = intStack[--isp];
							@Pc(6295) World local6295 = Static7.get(local157);
							if (local6295 != null) {
								intStack[isp++] = local6295.flags;
								stringStack[anInt3652++] = local6295.activity;
								@Pc(6319) Country local6319 = local6295.getCountry();
								intStack[isp++] = local6319.flag;
								stringStack[anInt3652++] = local6319.name;
								intStack[isp++] = local6295.anInt78;
								intStack[isp++] = local6295.anInt83;
								return;
							}
							intStack[isp++] = -1;
							stringStack[anInt3652++] = "";
							intStack[isp++] = 0;
							stringStack[anInt3652++] = "";
							intStack[isp++] = 0;
							intStack[isp++] = 0;
							return;
						}
						if (opcode == 6507) {
							isp -= 4;
							local157 = intStack[isp];
							local1292 = intStack[isp + 1] == 1;
							local89 = intStack[isp + 2];
							local1903 = intStack[isp + 3] == 1;
							Static343.method5783(local1292, local1903, local89, local157);
							return;
						}
						if (opcode == 6508) {
							Static200.method3643();
							return;
						}
						if (opcode == 6509) {
							if (client.gameState != 10) {
								return;
							}
							Static36.aBoolean74 = intStack[--isp] == 1;
							return;
						}
					} else if (opcode < 6700) {
						if (opcode == 6600) {
							Static218.preferences.aBoolean351 = intStack[--isp] == 1;
							Static218.preferences.write(GameShell.signlink);
							return;
						}
						if (opcode == 6601) {
							intStack[isp++] = Static218.preferences.aBoolean351 ? 1 : 0;
							return;
						}
					} else if (opcode < 6800 && client.modeWhat == GameMode.GAME_MODE_WIP) {
						if (opcode == 6700) {
							local157 = Static329.subInterfaces.method83();
							if (Static139.topLevelInterface != -1) {
								local157++;
							}
							intStack[isp++] = local157;
							return;
						}
						if (opcode == 6701) {
							local157 = intStack[--isp];
							if (Static139.topLevelInterface != -1) {
								if (local157 == 0) {
									intStack[isp++] = Static139.topLevelInterface;
									return;
								}
								local157--;
							}
							@Pc(6577) SubInterface local6577 = (SubInterface) Static329.subInterfaces.head();
							while (local157-- > 0) {
								local6577 = (SubInterface) Static329.subInterfaces.next();
							}
							intStack[isp++] = local6577.id;
							return;
						}
						if (opcode == 6702) {
							local157 = intStack[--isp];
							if (Static26.aComponentArrayArray1[local157] == null) {
								stringStack[anInt3652++] = "";
								return;
							}
							local81 = Static26.aComponentArrayArray1[local157][0].aString43;
							if (local81 == null) {
								stringStack[anInt3652++] = "";
								return;
							}
							stringStack[anInt3652++] = local81.substring(0, local81.indexOf(58));
							return;
						}
						if (opcode == 6703) {
							local157 = intStack[--isp];
							if (Static26.aComponentArrayArray1[local157] == null) {
								intStack[isp++] = 0;
								return;
							}
							intStack[isp++] = Static26.aComponentArrayArray1[local157].length;
							return;
						}
						if (opcode == 6704) {
							isp -= 2;
							local157 = intStack[isp];
							local192 = intStack[isp + 1];
							if (Static26.aComponentArrayArray1[local157] == null) {
								stringStack[anInt3652++] = "";
								return;
							}
							local3230 = Static26.aComponentArrayArray1[local157][local192].aString43;
							if (local3230 == null) {
								stringStack[anInt3652++] = "";
								return;
							}
							stringStack[anInt3652++] = local3230;
							return;
						}
						if (opcode == 6705) {
							isp -= 2;
							local157 = intStack[isp];
							local192 = intStack[isp + 1];
							if (Static26.aComponentArrayArray1[local157] == null) {
								intStack[isp++] = 0;
								return;
							}
							intStack[isp++] = Static26.aComponentArrayArray1[local157][local192].anInt4246;
							return;
						}
						if (opcode == 6706) {
							return;
						}
						if (opcode == 6707) {
							isp -= 3;
							local157 = intStack[isp];
							local192 = intStack[isp + 1];
							local89 = intStack[isp + 2];
							Static277.method4680(local157 << 16 | local192, 1, "", local89);
							return;
						}
						if (opcode == 6708) {
							isp -= 3;
							local157 = intStack[isp];
							local192 = intStack[isp + 1];
							local89 = intStack[isp + 2];
							Static277.method4680(local157 << 16 | local192, 2, "", local89);
							return;
						}
						if (opcode == 6709) {
							isp -= 3;
							local157 = intStack[isp];
							local192 = intStack[isp + 1];
							local89 = intStack[isp + 2];
							Static277.method4680(local157 << 16 | local192, 3, "", local89);
							return;
						}
						if (opcode == 6710) {
							isp -= 3;
							local157 = intStack[isp];
							local192 = intStack[isp + 1];
							local89 = intStack[isp + 2];
							Static277.method4680(local157 << 16 | local192, 4, "", local89);
							return;
						}
						if (opcode == 6711) {
							isp -= 3;
							local157 = intStack[isp];
							local192 = intStack[isp + 1];
							local89 = intStack[isp + 2];
							Static277.method4680(local157 << 16 | local192, 5, "", local89);
							return;
						}
						if (opcode == 6712) {
							isp -= 3;
							local157 = intStack[isp];
							local192 = intStack[isp + 1];
							local89 = intStack[isp + 2];
							Static277.method4680(local157 << 16 | local192, 6, "", local89);
							return;
						}
						if (opcode == 6713) {
							isp -= 3;
							local157 = intStack[isp];
							local192 = intStack[isp + 1];
							local89 = intStack[isp + 2];
							Static277.method4680(local157 << 16 | local192, 7, "", local89);
							return;
						}
						if (opcode == 6714) {
							isp -= 3;
							local157 = intStack[isp];
							local192 = intStack[isp + 1];
							local89 = intStack[isp + 2];
							Static277.method4680(local157 << 16 | local192, 8, "", local89);
							return;
						}
						if (opcode == 6715) {
							isp -= 3;
							local157 = intStack[isp];
							local192 = intStack[isp + 1];
							local89 = intStack[isp + 2];
							Static277.method4680(local157 << 16 | local192, 9, "", local89);
							return;
						}
						if (opcode == 6716) {
							isp -= 3;
							local157 = intStack[isp];
							local192 = intStack[isp + 1];
							local89 = intStack[isp + 2];
							Static277.method4680(local157 << 16 | local192, 10, "", local89);
							return;
						}
						if (opcode == 6717) {
							isp -= 3;
							local157 = intStack[isp];
							local192 = intStack[isp + 1];
							local89 = intStack[isp + 2];
							@Pc(7165) Component local7165 = Static207.getCreatedComponent(local157 << 16 | local192, local89);
							Static207.method3698();
							@Pc(7170) ServerActiveProperties local7170 = Static45.getServerActiveProperties(local7165);
							Static185.method3399(local7165.anInt4238, local7170.getTargetMask(), local157 << 16 | local192, local89, local7165.anInt4286, local7170.anInt1759);
							return;
						}
					} else if (opcode < 6900) {
						@Pc(7207) Class164 local7207;
						if (opcode == 6800) {
							local157 = intStack[--isp];
							local7207 = Static236.aClass125_1.method3378(local157);
							if (local7207.aString48 == null) {
								stringStack[anInt3652++] = "";
								return;
							}
							stringStack[anInt3652++] = local7207.aString48;
							return;
						}
						if (opcode == 6801) {
							local157 = intStack[--isp];
							local7207 = Static236.aClass125_1.method3378(local157);
							intStack[isp++] = local7207.anInt4420;
							return;
						}
						if (opcode == 6802) {
							local157 = intStack[--isp];
							local7207 = Static236.aClass125_1.method3378(local157);
							intStack[isp++] = local7207.anInt4418;
							return;
						}
						if (opcode == 6803) {
							local157 = intStack[--isp];
							local7207 = Static236.aClass125_1.method3378(local157);
							intStack[isp++] = local7207.anInt4424;
							return;
						}
						if (opcode == 6804) {
							isp -= 2;
							local157 = intStack[isp];
							local192 = intStack[isp + 1];
							@Pc(7329) Class240 local7329 = Static363.aClass18_2.method565(local192);
							if (local7329.method6262()) {
								stringStack[anInt3652++] = Static236.aClass125_1.method3378(local157).method4184(local192, local7329.aString69);
								return;
							}
							intStack[isp++] = Static236.aClass125_1.method3378(local157).method4187(local7329.anInt7023, local192);
							return;
						}
					}
				}
			}
		}
		throw new IllegalStateException();
	}

	@OriginalMember(owner = "client!lk", name = "a", descriptor = "(Lclient!js;I)V")
	private static void run(@OriginalArg(0) ClientScript arg0, @OriginalArg(1) int maxCycles) {
		isp = 0;
		anInt3652 = 0;
		@Pc(5) int local5 = -1;
		@Pc(8) int[] local8 = arg0.opcodes;
		@Pc(11) int[] local11 = arg0.intOperands;
		@Pc(13) byte local13 = -1;
		anInt3661 = 0;
		@Pc(603) int local603;
		try {
			@Pc(17) int local17 = 0;
			label260: while (true) {
				local17++;
				if (local17 > maxCycles) {
					throw new RuntimeException("slow");
				}
				local5++;
				@Pc(31) int local31 = local8[local5];
				if (local31 >= 100) {
					@Pc(802) boolean local802;
					if (local11[local5] == 1) {
						local802 = true;
					} else {
						local802 = false;
					}
					if (local31 >= 100 && local31 < 5000) {
						run(local31, local802);
					} else if (local31 >= 5000 && local31 < 10000) {
						method3598(local31, local802);
					} else {
						throw new IllegalStateException("Command: " + local31);
					}
				} else if (local31 == 0) {
					intStack[isp++] = local11[local5];
				} else {
					@Pc(54) int local54;
					if (local31 == 1) {
						local54 = local11[local5];
						intStack[isp++] = Static214.aClass226_1.anIntArray430[local54];
					} else if (local31 == 2) {
						local54 = local11[local5];
						Static214.aClass226_1.method5779(local54, intStack[--isp]);
					} else if (local31 == 3) {
						stringStack[anInt3652++] = arg0.stringOperands[local5];
					} else if (local31 == 6) {
						local5 += local11[local5];
					} else if (local31 == 7) {
						isp -= 2;
						if (intStack[isp] != intStack[isp + 1]) {
							local5 += local11[local5];
						}
					} else if (local31 == 8) {
						isp -= 2;
						if (intStack[isp] == intStack[isp + 1]) {
							local5 += local11[local5];
						}
					} else if (local31 == 9) {
						isp -= 2;
						if (intStack[isp] < intStack[isp + 1]) {
							local5 += local11[local5];
						}
					} else if (local31 == 10) {
						isp -= 2;
						if (intStack[isp] > intStack[isp + 1]) {
							local5 += local11[local5];
						}
					} else if (local31 == 21) {
						if (anInt3661 == 0) {
							return;
						}
						@Pc(216) Class172 local216 = aClass172Array1[--anInt3661];
						arg0 = local216.aClientScript_1;
						local8 = arg0.opcodes;
						local11 = arg0.intOperands;
						local5 = local216.anInt4704;
						intLocals = local216.anIntArray319;
						stringLocals = local216.aStringArray47;
					} else if (local31 == 25) {
						local54 = local11[local5];
						intStack[isp++] = Static214.aClass226_1.method5777(local54);
					} else if (local31 == 27) {
						local54 = local11[local5];
						Static214.aClass226_1.method5778(local54, intStack[--isp]);
					} else if (local31 == 31) {
						isp -= 2;
						if (intStack[isp] <= intStack[isp + 1]) {
							local5 += local11[local5];
						}
					} else if (local31 == 32) {
						isp -= 2;
						if (intStack[isp] >= intStack[isp + 1]) {
							local5 += local11[local5];
						}
					} else if (local31 == 33) {
						intStack[isp++] = intLocals[local11[local5]];
					} else if (local31 == 34) {
						intLocals[local11[local5]] = intStack[--isp];
					} else if (local31 == 35) {
						stringStack[anInt3652++] = stringLocals[local11[local5]];
					} else if (local31 == 36) {
						stringLocals[local11[local5]] = stringStack[--anInt3652];
					} else if (local31 == 37) {
						local54 = local11[local5];
						anInt3652 -= local54;
						@Pc(400) String local400 = Static89.method1940(anInt3652, stringStack, local54);
						stringStack[anInt3652++] = local400;
					} else if (local31 == 38) {
						isp--;
					} else if (local31 == 39) {
						anInt3652--;
					} else {
						@Pc(452) int local452;
						if (local31 == 40) {
							local54 = local11[local5];
							@Pc(436) ClientScript local436 = Static295.get(local54);
							if (local436 == null) {
								throw new RuntimeException();
							}
							@Pc(446) int[] local446 = new int[local436.intLocals];
							@Pc(450) String[] local450 = new String[local436.stringLocals];
							for (local452 = 0; local452 < local436.intArgs; local452++) {
								local446[local452] = intStack[isp + local452 - local436.intArgs];
							}
							for (@Pc(471) int local471 = 0; local471 < local436.stringArgs; local471++) {
								local450[local471] = stringStack[anInt3652 + local471 - local436.stringArgs];
							}
							isp -= local436.intArgs;
							anInt3652 -= local436.stringArgs;
							@Pc(502) Class172 local502 = new Class172();
							local502.aClientScript_1 = arg0;
							local502.anInt4704 = local5;
							local502.anIntArray319 = intLocals;
							local502.aStringArray47 = stringLocals;
							if (anInt3661 >= aClass172Array1.length) {
								throw new RuntimeException();
							}
							aClass172Array1[anInt3661++] = local502;
							arg0 = local436;
							local8 = local436.opcodes;
							local11 = local436.intOperands;
							local5 = -1;
							intLocals = local446;
							stringLocals = local450;
						} else if (local31 == 42) {
							intStack[isp++] = Static75.anIntArray117[local11[local5]];
						} else if (local31 == 43) {
							local54 = local11[local5];
							Static75.anIntArray117[local54] = intStack[--isp];
							Static226.method3982(local54);
							Static273.aBoolean374 |= Static89.aBooleanArray18[local54];
						} else if (local31 == 44) {
							local54 = local11[local5] >> 16;
							local603 = local11[local5] & 0xFFFF;
							@Pc(611) int local611 = intStack[--isp];
							if (local611 >= 0 && local611 <= 5000) {
								anIntArray214[local54] = local611;
								@Pc(626) byte local626 = -1;
								if (local603 == 105) {
									local626 = 0;
								}
								local452 = 0;
								while (true) {
									if (local452 >= local611) {
										continue label260;
									}
									arrays[local54][local452] = local626;
									local452++;
								}
							}
							throw new RuntimeException();
						} else if (local31 == 45) {
							local54 = local11[local5];
							local603 = intStack[--isp];
							if (local603 < 0 || local603 >= anIntArray214[local54]) {
								throw new RuntimeException();
							}
							intStack[isp++] = arrays[local54][local603];
						} else if (local31 == 46) {
							local54 = local11[local5];
							isp -= 2;
							local603 = intStack[isp];
							if (local603 < 0 || local603 >= anIntArray214[local54]) {
								throw new RuntimeException();
							}
							arrays[local54][local603] = intStack[isp + 1];
						} else if (local31 == 47) {
							@Pc(730) String local730 = Static75.aStringArray11[local11[local5]];
							if (local730 == null) {
								local730 = "null";
							}
							stringStack[anInt3652++] = local730;
						} else if (local31 == 48) {
							local54 = local11[local5];
							Static75.aStringArray11[local54] = stringStack[--anInt3652];
							Static290.method5022(local54);
						} else if (local31 == 51) {
							@Pc(774) HashTable local774 = arg0.switchTables[local11[local5]];
							@Pc(787) IntNode local787 = (IntNode) local774.get((long) intStack[--isp]);
							if (local787 != null) {
								local5 += local787.value;
							}
						}
					}
				}
			}
		} catch (@Pc(837) Exception local837) {
			@Pc(855) StringBuffer local855;
			if (arg0.name == null) {
				local855 = new StringBuffer(30);
				local855.append("CS2: ").append(arg0.key).append(" ");
				for (local603 = anInt3661 - 1; local603 >= 0; local603--) {
					local855.append("v: ").append(aClass172Array1[local603].aClientScript_1.key).append(" ");
				}
				local855.append("op: ").append(local13);
				Static262.report(local837, local855.toString());
			} else {
				Static207.add("Clientscript error in: " + arg0.name);
				local855 = new StringBuffer(30);
				local855.append("Clientscript error in: ").append(arg0.name).append("\n");
				for (local603 = anInt3661 - 1; local603 >= 0; local603--) {
					local855.append("via: ").append(aClass172Array1[local603].aClientScript_1.name).append("\n");
				}
				local855.append("Op: ").append(local13).append("\n");
				@Pc(895) String local895 = local837.getMessage();
				if (local895 != null && local895.length() > 0) {
					local855.append("Message: ").append(local895).append("\n");
				}
				Static262.report(local837, local855.toString());
				Static158.add(local855.toString());
			}
		}
	}
}
