...
public final class TextureOpPerlinNoise extends TextureOp {
	...
	public void decode(@OriginalArg(1) Buffer buffer, @OriginalArg(0) int opcode) {
		if (opcode == 0) {
			this.aBoolean85 = buffer.g1() == 1;
		} else if (opcode == 1) {
			this.anInt1043 = buffer.g1();
		} else if (opcode == 2) {
			this.anInt1049 = buffer.g2b_dup();
			if (this.anInt1049 < 0) {
				this.aShortArray18 = new short[this.anInt1043];
				for (@Pc(80) int local80 = 0; local80 < this.anInt1043; local80++) {
					this.aShortArray18[local80] = (short) buffer.g2b_dup();
				}
				return;
			}
		} else if (opcode == 3) {
			this.anInt1051 = this.anInt1050 = buffer.g1();
			return;
		} else if (opcode == 4) {
			this.anInt1045 = buffer.g1();
			return;
		} else if (opcode == 5) {
			this.anInt1051 = buffer.g1();
			return;
		} else if (opcode == 6) {
			this.anInt1050 = buffer.g1();
			return;
		}
	}
