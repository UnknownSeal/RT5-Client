	...
	public static Class2_Sub2_Sub11 method3744(@OriginalArg(0) byte[] arg0) {
		...
		if (local74 > 0) {
			...
			for (local85 = 0; local85 < local74; local85++) {
				local91 = local21.g2();
				@Pc(98) HashTable local98 = new HashTable(Static162.clp2(local91));
				local16.aClass4Array1[local85] = local98;
				while (local91-- > 0) {
					@Pc(109) int local109 = local21.mg4();
					@Pc(115) int local115 = local21.mg4();
					local98.put(new IntNode(local115), (long) local109);
				}
			}
		}
		local21.offset = 0;
		...
		while (local43 > local21.offset) {
			...
			if (local91 == 3) {
				...
				local16.anIntArray200[local85] = local21.mg4();
			}
